...
void VectorTransform::reverse_transform(idx_t, const float*, float*) const {
    FAISS_NAVIX_THROW_MSG("reverse transform not implemented");
}

void VectorTransform::check_identical(const VectorTransform& other) const {
    FAISS_NAVIX_THROW_IF_NOT(other.d_in == d_in && other.d_in == d_in);
}
...
void LinearTransform::apply_noalloc(idx_t n, const float* x, float* xt) const {
    FAISS_NAVIX_THROW_IF_NOT_MSG(is_trained, "Transformation not trained yet");

    float c_factor;
    if (have_bias) {
        FAISS_NAVIX_THROW_IF_NOT_MSG(b.size() == d_out, "Bias not initialized");
        float* xi = xt;
        for (int i = 0; i < n; i++)
...
void LinearTransform::set_is_orthonormal() {
    ...
    double eps = 4e-5;
    FAISS_NAVIX_ASSERT(A.size() >= d_out * d_in);
    {
...
void LinearTransform::print_if_verbose(
    ...
    if (!verbose)
        return;
    printf("matrix %s: %d*%d [\n", name, n, d);
    FAISS_NAVIX_THROW_IF_NOT(mat.size() >= n * d);
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < d; j++) {
...
void LinearTransform::check_identical(const VectorTransform& other_in) const {
    ...
    FAISS_NAVIX_THROW_IF_NOT(other->A == A && other->b == b);
}
...
void PCAMatrix::copy_from(const PCAMatrix& other) {
    FAISS_NAVIX_THROW_IF_NOT(other.is_trained);
    mean = other.mean;
...
void PCAMatrix::prepare_Ab() {
    ...
    if (!random_rotation) {
        ...
        if (balanced_bins != 0) {
            FAISS_NAVIX_THROW_IF_NOT(d_out % balanced_bins == 0);
...
void ITQMatrix::train(idx_t n, const float* xf) {
    ...
    for (int i = 0; i < max_iter; i++) {
        ...
        // SVD
        {
            ...
            FAISS_NAVIX_THROW_IF_NOT(info == 0);
            lwork = size_t(lwork1);
            ...
            dgesvd_("A",
                    ...
                    &lwork,
                    &info);
            FAISS_NAVIX_THROW_IF_NOT_FMT(info == 0, "sgesvd returned info=%d", info);
        }
...
ITQTransform::ITQTransform(int d_in, int d_out, bool do_pca)
    ...
    if (!do_pca) {
        FAISS_NAVIX_THROW_IF_NOT(d_in == d_out);
    }
...
void ITQTransform::check_identical(const VectorTransform& other_in) const {
    ...
    FAISS_NAVIX_THROW_IF_NOT(other->mean == mean);
}
...
void OPQMatrix::train(idx_t n, const float* x_in) {
    ...
    if (A.size() == 0) {
        ...
        FAISS_NAVIX_THROW_IF_NOT(A.size() == d * d2);
        rotation = A.data();
    }
...
void NormalizationTransform::apply_noalloc(idx_t n, const float* x, float* xt)
    ...
    if (norm == 2.0) {
        ...
        FAISS_NAVIX_THROW_MSG("not implemented");
    }
}
...
void NormalizationTransform::check_identical(
    ...
    FAISS_NAVIX_THROW_IF_NOT(other);
    FAISS_NAVIX_THROW_IF_NOT(other->norm == norm);
}
...
void CenteringTransform::train(idx_t n, const float* x) {
    FAISS_NAVIX_THROW_IF_NOT_MSG(n > 0, "need at least one training vector");
    mean.resize(d_in, 0);
    for (idx_t i = 0; i < n; i++) {
        for (size_t j = 0; j < d_in; j++) {
            mean[j] += *x++;
...
RemapDimensionsTransform::RemapDimensionsTransform(
    ...
    for (int i = 0; i < d_out; i++) {
        map[i] = map_in[i];
        FAISS_NAVIX_THROW_IF_NOT(map[i] == -1 || (map[i] >= 0 && map[i] < d_in));
    }
}
...
void RemapDimensionsTransform::check_identical(
    ...
    FAISS_NAVIX_THROW_IF_NOT(other);
    FAISS_NAVIX_THROW_IF_NOT(other->map == map);
}
