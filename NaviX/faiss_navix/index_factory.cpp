...
Index* parse_coarse_quantizer(
    ...
    if (match("IVF([0-9]+[kM]?)\\(Index([0-9])\\)")) {
        ...
        FAISS_NAVIX_ASSERT(no >= 0 && no < parenthesis_indexes.size());
...
IndexIVF* parse_IndexIVF(
    ...
    if (match("(RQ|LSQ)" + aq_def_pattern + aq_norm_pattern)) {
        ...
        if (sm[1].str() == "RQ") {
            ...
            FAISS_NAVIX_THROW_IF_NOT(nbits.size() > 0);
...
Index* parse_other_indexes(
    ...
    if (match("LSH([0-9]*)(r?)(t?)")) {
        ...
        FAISS_NAVIX_THROW_IF_NOT(metric == METRIC_L2);
...
std::unique_ptr<Index> index_factory_sub(
    ...
    if (re_match(description, "(.+),RFlat", sm) ||
        ...
        FAISS_NAVIX_ASSERT(index_rf != nullptr);
    ...
    FAISS_NAVIX_THROW_FMT("could not parse index string %s", description.c_str());
    return nullptr;
}
