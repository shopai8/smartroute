...
void IndexSplitVectors::sync_with_sub_indexes() {
    ...
    for (int i = 1; i < sub_indexes.size(); i++) {
        ...
        FAISS_NAVIX_THROW_IF_NOT(metric_type == index->metric_type);
        FAISS_NAVIX_THROW_IF_NOT(ntotal == index->ntotal);
        sum_d += index->d;
    }
}
...
void IndexSplitVectors::search(
    ...
    FAISS_NAVIX_THROW_IF_NOT_MSG(k == 1, "search implemented only for k=1");
    FAISS_NAVIX_THROW_IF_NOT_MSG(
...
void IndexRandom::search(
    ...
    FAISS_NAVIX_THROW_IF_NOT_MSG(
            !params, "search params not supported for this index");
    FAISS_NAVIX_THROW_IF_NOT(k <= ntotal);
