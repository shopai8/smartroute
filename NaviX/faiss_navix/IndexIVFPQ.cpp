...
void initialize_IVFPQ_precomputed_table(
    ...
    size_t d = quantizer->d;
    FAISS_NAVIX_THROW_IF_NOT(d == pq.d);
    ...
    if (use_precomputed_table == 1) {
        ...
        const MultiIndexQuantizer* miq =
                dynamic_cast<const MultiIndexQuantizer*>(quantizer);
        FAISS_NAVIX_THROW_IF_NOT(miq);
        const ProductQuantizer& cpq = miq->pq;
        FAISS_NAVIX_THROW_IF_NOT(pq.M % cpq.M == 0);
...
struct QueryTables {
    ...
    float precompute_list_table_pointers_L2() {
        ...
        if (use_precomputed_table == 1) {
            ...
            FAISS_NAVIX_THROW_MSG("need precomputed tables");
        }
...
struct IVFPQScanner : IVFPQScannerT<idx_t, METRIC_TYPE, PQDecoder>,
    ...
    size_t scan_codes(
        ...
        if (this->polysemous_ht > 0) {
            ...
            FAISS_NAVIX_THROW_MSG("bad precomp mode");
        }
        return res.nup;
    }
