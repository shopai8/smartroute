...
#include <faiss_navix/IndexFastScan.h>
...
void IndexFastScan::init_fastscan(
    ...
    FAISS_NAVIX_THROW_IF_NOT(bbs % 32 == 0);
    this->d = d;
    this->M = M_init;
...
void IndexFastScan::check_compatible_for_merge(const Index& otherIndex) const {
    ...
    FAISS_NAVIX_THROW_IF_NOT(other->M == M);
    FAISS_NAVIX_THROW_IF_NOT(other->bbs == bbs);
...
void IndexFastScan::search_dispatch_implem(
    ...
    if (implem == 1) {
        ...
        FAISS_NAVIX_THROW_IF_NOT(orig_codes != nullptr);
        search_implem_234<Cfloat>(n, x, k, distances, labels, scaler);
    } else if (impl >= 12 && impl <= 15) {
        FAISS_NAVIX_THROW_IF_NOT(ntotal < INT_MAX);
...
#pragma omp parallel for num_threads(nt)
        ...
        FAISS_NAVIX_THROW_FMT("invalid implem %d impl=%d", implem, impl);
    }
}
...
void IndexFastScan::search_implem_234(
        ...
        idx_t* labels,
        const NormTableScaler* scaler) const {
    FAISS_NAVIX_THROW_IF_NOT(implem == 2 || implem == 3 || implem == 4);
...
void IndexFastScan::search_implem_12(
    ...
    FAISS_NAVIX_THROW_IF_NOT(LUT_nq == n);
