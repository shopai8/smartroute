...
#pragma omp for schedule(static)
        ...
        if (index_hnsw.init_level0) {
            FAISS_NAVIX_ASSERT(i1 == 0);
        } else {
            FAISS_NAVIX_ASSERT((i1 - hist[0]) == 0);
        }
    }
    if (verbose) {
...
void IndexHNSW::link_singletons() {
    ...
    for (int i = 0; i < singletons.size(); i++) {
        FAISS_NAVIX_ASSERT(!"not implemented");
    }
}
        ...
        for (size_t j = begin; j < end; j++) {
            ...
            if (vt.visited[v1] == vt.visno + 1) {
                ...
                if (vt.visited[v1] < vt.visno) {
                    if (nres < k) {
                        faiss_navix::maxheap_push(++nres, D, I, d, v1);
                    } else if (d < D[0]) {
                        faiss_navix::maxheap_replace_top(nres, D, I, d, v1);
                    }
                }
                vt.visited[v1] = vt.visno + 1;
            }
...
void IndexHNSW2Level::flip_to_ivf() {
    ...
    FAISS_NAVIX_THROW_IF_NOT(storage2l);
