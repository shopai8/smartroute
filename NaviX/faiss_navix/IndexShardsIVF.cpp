...
void IndexShardsIVF::addIndex(Index* index) {
    ...
    FAISS_NAVIX_THROW_IF_NOT_MSG(index_ivf, "can only add IndexIVFs");
    FAISS_NAVIX_THROW_IF_NOT(index_ivf->nlist == nlist);
...
void IndexShardsIVF::search(
    ...
    FAISS_NAVIX_THROW_IF_NOT(count() > 0);
    ...
    auto fn = [&](int no, const Index* indexIn) {
        ...
        FAISS_NAVIX_THROW_IF_NOT_MSG(index->nprobe == nprobe, "inconsistent nprobe");
