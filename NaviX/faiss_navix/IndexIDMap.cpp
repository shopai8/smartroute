...
#include <faiss_navix/utils/WorkerThread.h>
...
IndexIDMapTemplate<IndexT>::IndexIDMapTemplate(IndexT* index) : index(index) {
    FAISS_NAVIX_THROW_IF_NOT_MSG(index->ntotal == 0, "index must be empty on input");
...
size_t IndexIDMapTemplate<IndexT>::remove_ids(const IDSelector& sel) {
    ...
    FAISS_NAVIX_ASSERT(j == index->ntotal);
    this->ntotal = j;
...
void IndexIDMap2Template<IndexT>::check_consistency() const {
    FAISS_NAVIX_THROW_IF_NOT(rev_map.size() == this->id_map.size());
    FAISS_NAVIX_THROW_IF_NOT(this->id_map.size() == this->ntotal);
    for (size_t i = 0; i < this->ntotal; i++) {
        idx_t ii = rev_map.at(this->id_map[i]);
        FAISS_NAVIX_THROW_IF_NOT(ii == i);
    }
}
...
void IndexIDMap2Template<IndexT>::reconstruct(
    ...
    try {
        ...
        FAISS_NAVIX_THROW_FMT("key %" PRId64 " not found", key);
    }
}
