...
#include <faiss_navix/impl/LocalSearchQuantizer.h>
#include <faiss_navix/impl/ProductQuantizer.h>
#include <faiss_navix/impl/ResidualQuantizer.h>
#include <faiss_navix/impl/ScalarQuantizer.h>
#include <faiss_navix/impl/pq4_fast_scan.h>

#include <faiss_navix/invlists/BlockInvertedLists.h>
...
VectorTransform* Cloner::clone_VectorTransform(const VectorTransform* vt) {
    ...
    TRYCLONE(LinearTransform, vt) {
        FAISS_NAVIX_THROW_MSG("clone not supported for this type of VectorTransform");
    }
    return nullptr;
}
...
IndexIVF* Cloner::clone_IndexIVF(const IndexIVF* ivf) {
    ...
    TRYCLONE(IndexIVFScalarQuantizer, ivf) {
        FAISS_NAVIX_THROW_MSG("clone not supported for this type of IndexIVF");
    }
    return nullptr;
}
...
IndexRefine* clone_IndexRefine(const IndexRefine* ir) {
    ...
    TRYCLONE(IndexRefine, ir) {
        FAISS_NAVIX_THROW_MSG("clone not supported for this type of IndexRefine");
    }
}
...
IndexIDMap* clone_IndexIDMap(const IndexIDMap* im) {
    ...
    TRYCLONE(IndexIDMap, im) {
        FAISS_NAVIX_THROW_MSG("clone not supported for this type of IndexIDMap");
    }
}
...
IndexHNSW* clone_IndexHNSW(const IndexHNSW* ihnsw) {
    ...
    TRYCLONE(IndexHNSW, ihnsw) {
        FAISS_NAVIX_THROW_MSG("clone not supported for this type of IndexHNSW");
    }
}
...
IndexNNDescent* clone_IndexNNDescent(const IndexNNDescent* innd) {
    ...
    TRYCLONE(IndexNNDescent, innd) {
        FAISS_NAVIX_THROW_MSG("clone not supported for this type of IndexNNDescent");
    }
}
...
InvertedLists* clone_InvertedLists(const InvertedLists* invlists) {
    ...
    if (auto* bils = dynamic_cast<const BlockInvertedLists*>(invlists)) {
        ...
        if (bils->packer) {
            ...
            FAISS_NAVIX_THROW_IF_NOT(packerPQ4);
...
Quantizer* clone_Quantizer(const Quantizer* quant) {
    ...
    FAISS_NAVIX_THROW_MSG("Did not recognize quantizer to clone");
}
