...
size_t InvertedLists::copy_subset_to(
        ...
        idx_t a1,
        idx_t a2) const {
    FAISS_NAVIX_THROW_IF_NOT(nlist == oivf.nlist);
    FAISS_NAVIX_THROW_IF_NOT(code_size == oivf.code_size);
...
double InvertedLists::imbalance_factor() const {
    ...
    return faiss_navix::imbalance_factor(nlist, hist.data());
}
...
bool InvertedLists::is_empty(size_t list_no, void* inverted_list_context)
    ...
    if (use_iterator) {
        ...
        FAISS_NAVIX_THROW_IF_NOT(inverted_list_context == nullptr);
...
void ArrayInvertedLists::permute_invlists(const idx_t* map) {
    ...
    for (size_t i = 0; i < nlist; i++) {
        size_t o = map[i];
        FAISS_NAVIX_THROW_IF_NOT(o < nlist);
...
HStackInvertedLists::HStackInvertedLists(int nil, const InvertedLists** ils_in)
    ...
    FAISS_NAVIX_THROW_IF_NOT(nil > 0);
    for (int i = 0; i < nil; i++) {
...
const uint8_t* HStackInvertedLists::get_single_code(
    ...
    FAISS_NAVIX_THROW_FMT("offset %zd unknown", offset);
}
...
idx_t translate_list_no(const SliceInvertedLists* sil, idx_t list_no) {
    FAISS_NAVIX_THROW_IF_NOT(list_no >= 0 && list_no < sil->nlist);
...
int translate_list_no(const VStackInvertedLists* vil, idx_t list_no) {
    FAISS_NAVIX_THROW_IF_NOT(list_no >= 0 && list_no < vil->nlist);
...
VStackInvertedLists::VStackInvertedLists(int nil, const InvertedLists** ils_in)
    ...
    for (int i = 0; i < nil; i++) {
        ils.push_back(ils_in[i]);
        FAISS_NAVIX_THROW_IF_NOT(ils_in[i]->code_size == code_size);
...
MaskedInvertedLists::MaskedInvertedLists(
        ...
        : ReadOnlyInvertedLists(il0->nlist, il0->code_size),
          il0(il0),
          il1(il1) {
    FAISS_NAVIX_THROW_IF_NOT(il1->nlist == nlist);
    FAISS_NAVIX_THROW_IF_NOT(il1->code_size == code_size);
}
...
idx_t StopWordsInvertedLists::get_single_id(size_t list_no, size_t offset)
        const {
    FAISS_NAVIX_THROW_IF_NOT(il0->list_size(list_no) < maxsize);
