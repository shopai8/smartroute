...
idx_t DirectMap::get(idx_t key) const {
    if (type == Array) {
        FAISS_NAVIX_THROW_IF_NOT_MSG(key >= 0 && key < array.size(), "invalid key");
        idx_t lo = array[key];
        FAISS_NAVIX_THROW_IF_NOT_MSG(lo >= 0, "-1 entry in direct_map");
        return lo;
    } else if (type == Hashtable) {
        auto res = hashtable.find(key);
        FAISS_NAVIX_THROW_IF_NOT_MSG(res != hashtable.end(), "key not found");
        return res->second;
    } else {
        FAISS_NAVIX_THROW_MSG("direct map not initialized");
    }
}
...
void DirectMap::check_can_add(const idx_t* ids) {
    if (type == Array && ids) {
        FAISS_NAVIX_THROW_MSG("cannot have array direct map and add with ids");
    }
}
...
DirectMapAdd::DirectMapAdd(DirectMap& direct_map, size_t n, const idx_t* xids)
    ...
    if (type == DirectMap::Array) {
        FAISS_NAVIX_THROW_IF_NOT(xids == nullptr);
...
#pragma omp parallel for
        ...
        FAISS_NAVIX_THROW_MSG("remove not supported with this direct_map format");
    }
    return nremove;
}
...
void DirectMap::update_codes(
        ...
        const idx_t* assign,
        const uint8_t* codes) {
    FAISS_NAVIX_THROW_IF_NOT(type == Array);
