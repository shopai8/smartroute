...
void OnDiskInvertedLists::update_totsize(size_t new_size) {
    ...
    if (ptr != nullptr) {
        ...
        FAISS_NAVIX_THROW_IF_NOT_FMT(err == 0, "munmap error: %s", strerror(errno));
    }
    if (totsize == 0) {
...
void OnDiskInvertedLists::update_entries(
    ...
    FAISS_NAVIX_THROW_IF_NOT(!read_only);
    if (n_entry == 0)
        return;
...
size_t OnDiskInvertedLists::merge_from_multiple(
    ...
    for (int i = 0; i < n_il; i++) {
        ...
        FAISS_NAVIX_THROW_IF_NOT(il->nlist == nlist && il->code_size == code_size);
...
void OnDiskInvertedLists::crop_invlists(size_t l0, size_t l1) {
    FAISS_NAVIX_THROW_IF_NOT(0 <= l0 && l0 <= l1 && l1 <= nlist);
...
InvertedLists* OnDiskInvertedListsIOHook::read_ArrayInvertedLists(
    ...
    FileIOReader* reader = dynamic_cast<FileIOReader*>(f);
    FAISS_NAVIX_THROW_IF_NOT_MSG(reader, "mmap only supported for File objects");
    FILE* fdesc = reader->f;
    ...
    { // do the mmap
        ...
        FAISS_NAVIX_THROW_IF_NOT_FMT(ret == 0, "fstat failed: %s", strerror(errno));
    ...
    FAISS_NAVIX_THROW_IF_NOT(o <= ails->totsize);
