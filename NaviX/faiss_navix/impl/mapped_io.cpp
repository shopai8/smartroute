...
struct MmappedFileMappingOwner::PImpl {
    ...
    PImpl(FILE* f) {
        ...
        if (fd == -1) {
            // no good
            FAISS_NAVIX_THROW_FMT("could not get a HANDLE");
        }

        HANDLE file_handle = (HANDLE)_get_osfhandle(fd);
        if (file_handle == INVALID_HANDLE_VALUE) {
            FAISS_NAVIX_THROW_FMT("could not get an OS HANDLE");
        }
        ...
        if (GetFileSizeEx(file_handle, &len_li) == 0) {
            const auto error = GetLastError();
            FAISS_NAVIX_THROW_FMT("could not get the file size (error %d)", error);
        }
        ...
        if (data == nullptr) {
            ...
            FAISS_NAVIX_THROW_FMT("could not get map the file, (error %d)", error);
        }
...
struct MmappedFileMappingOwner::PImpl {
    PImpl(FILE* f) {
        FAISS_NAVIX_THROW_FMT("Not implemented");
    }
