...
struct HNSW {
    ...
    struct MinimaxHeap {
        ...
        typedef faiss_navix::CMax<float, storage_idx_t> HC;
    ...
    faiss_navix::RandomGenerator rng;
    ...
    inline void navix_push_to_results(int k, float *D, idx_t *I, float dist, idx_t v, int &nres) const {
        if (nres < k) {
            faiss_navix::maxheap_push(++nres, D, I, dist, v);
        } else if (dist < D[0]) {
            faiss_navix::maxheap_replace_top(nres, D, I, dist, v);
        }
    }
...
FAISS_NAVIX_API extern HNSWStats hnsw_stats;
