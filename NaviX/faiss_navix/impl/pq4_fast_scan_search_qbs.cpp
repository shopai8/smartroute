...
#include <faiss_navix/utils/simdlib.h>
...
FAISS_NAVIX_PRAGMA_IMPRECISE_FUNCTION_BEGIN
...
#define DISPATCH(NQ)                                     \
    ...
    FAISS_NAVIX_THROW_FMT("accumulate nq=%d not instantiated", nq);
...
void accumulate_to_mem(
        ...
        const uint8_t* LUT,
        uint16_t* accu) {
    FAISS_NAVIX_THROW_IF_NOT(ntotal2 % 32 == 0);
...
int pq4_preferred_qbs(int n) {
    ...
    if (n <= 11) {
        ...
        FAISS_NAVIX_THROW_FMT("number of queries %d too large", n);
    }
}
