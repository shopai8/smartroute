...
#include <faiss_navix/impl/index_read_utils.h>
#include <faiss_navix/index_io.h>

#include <faiss_navix/impl/io_macros.h>
...
#include <faiss_navix/invlists/InvertedListsIOHook.h>

#include <faiss_navix/Index2Layer.h>
#include <faiss_navix/IndexAdditiveQuantizer.h>
#include <faiss_navix/IndexAdditiveQuantizerFastScan.h>
...
#include <faiss_navix/IndexIVFAdditiveQuantizerFastScan.h>
...
#include <faiss_navix/IndexIVFPQR.h>
#include <faiss_navix/IndexIVFSpectralHash.h>
#include <faiss_navix/IndexLSH.h>
#include <faiss_navix/IndexLattice.h>
#include <faiss_navix/IndexNNDescent.h>
...
#include <faiss_navix/VectorTransform.h>

#include <faiss_navix/IndexBinaryFlat.h>
#include <faiss_navix/IndexBinaryFromFloat.h>
#include <faiss_navix/IndexBinaryHNSW.h>
#include <faiss_navix/IndexBinaryHash.h>
#include <faiss_navix/IndexBinaryIVF.h>

// mmap-ing and viewing facilities
#include <faiss_navix/impl/maybe_owned_vector.h>

#include <faiss_navix/impl/mapped_io.h>
#include <faiss_navix/impl/zerocopy_io.h>
...
VectorTransform* read_VectorTransform(IOReader* f) {
    ...
    if (h == fourcc("rrot") || h == fourcc("PCAm") || h == fourcc("LTra") ||
        ...
        READVECTOR(lt->A);
        READVECTOR(lt->b);
        FAISS_NAVIX_THROW_IF_NOT(lt->A.size() >= lt->d_in * lt->d_out);
        FAISS_NAVIX_THROW_IF_NOT(!lt->have_bias || lt->b.size() >= lt->d_out);
...
void read_InvertedLists(IndexIVF* ivf, IOReader* f, int io_flags) {
    ...
    if (ils) {
        FAISS_NAVIX_THROW_IF_NOT(ils->nlist == ivf->nlist);
...
Index* read_index(IOReader* f, int io_flags) {
    ...
    if (h == fourcc("null")) {
        ...
        }
        {
            ...
            FAISS_NAVIX_THROW_IF_NOT_MSG(rrot, "expected a random rotation");
            idxl->rrot = *rrot;
            delete rrot;
        }
        ...
        FAISS_NAVIX_THROW_IF_NOT(indep->index_ivf);
...
static void read_binary_hash_invlists(
    ...
    for (size_t i = 0; i < sz; i++) {
        ...
        READVECTOR(il.ids);
        FAISS_NAVIX_THROW_IF_NOT(il.ids.size() == ilsz);
        READVECTOR(il.vecs);
    }
}
...
static void read_binary_multi_hash_map(
    ...
    FAISS_NAVIX_THROW_IF_NOT(buf.size() == (nbit + 7) / 8);
    BitstringReader rd(buf.data(), buf.size());
    map.reserve(sz);
    for (size_t i = 0; i < sz; i++) {
        uint64_t hash = rd.read(b);
...
IndexBinary* read_index_binary(IOReader* f, int io_flags) {
    ...
    if (h == fourcc("IBxF")) {
        ...
        FAISS_NAVIX_THROW_IF_NOT(idxf->xb.size() == idxf->ntotal * idxf->code_size);
        // leak!
        idx = idxf;
