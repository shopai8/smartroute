...
#include <faiss_navix/utils/hamming.h> // BitstringWriter
#include <faiss_navix/utils/utils.h>

#include <faiss_navix/utils/approx_topk/approx_topk.h>
...
void LocalSearchQuantizer::train(size_t n, const float* x) {
    FAISS_NAVIX_THROW_IF_NOT(K == (1 << nbits[0]));
...
void LocalSearchQuantizer::compute_codes_add_centroids(
    ...
    FAISS_NAVIX_THROW_IF_NOT_MSG(is_trained, "LSQ is not trained yet.");
...
void LocalSearchQuantizer::icm_encode_impl(
    ...
    FAISS_NAVIX_THROW_IF_NOT(nperts <= M);
...
void LocalSearchQuantizer::icm_encode_step(
    ...
    FAISS_NAVIX_THROW_IF_NOT(M != 0 && K != 0);
    FAISS_NAVIX_THROW_IF_NOT(binaries != nullptr);
...
#pragma omp parallel for reduction(+ : obj)
    for (int64_t i = 0; i < n; i++) {
        ...
        float err = faiss_navix::fvec_L2sqr(x + i * d, decoded_i, d);
        obj += err;
