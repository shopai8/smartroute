...
void pq4_accumulate_loop_fixed_scaler(
        ...
        ResultHandler& res,
        const Scaler& scaler) {
    FAISS_NAVIX_THROW_IF_NOT(is_aligned_pointer(codes));
    FAISS_NAVIX_THROW_IF_NOT(is_aligned_pointer(LUT));
    FAISS_NAVIX_THROW_IF_NOT(bbs % 32 == 0);
    FAISS_NAVIX_THROW_IF_NOT(nb % bbs == 0);
...
#define DISPATCH(NQ, BB)                                                   \
    ...
    switch (nq * 1000 + bbs / 32) {
        ...
        DISPATCH(4, 1);
        default:
            FAISS_NAVIX_THROW_FMT("nq=%d bbs=%d not instantiated", nq, bbs);
    }
#undef DISPATCH
}
