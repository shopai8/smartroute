...
SimulatedAnnealingOptimizer::SimulatedAnnealingOptimizer(
    ...
    FAISS_NAVIX_THROW_IF_NOT(n < 100000 && n >= 0);
}
...
struct ReproduceWithHammingObjective : PermutationObjective {
    ...
    ReproduceWithHammingObjective(
        ...
        n = 1 << nbits;
        FAISS_NAVIX_THROW_IF_NOT(dis_table.size() == n * n);
...
#pragma omp parallel for num_threads(nt)
    for (int m = 0; m < pq.M; m++) {
        ...
        if (log_pattern.size()) {
            ...
            FAISS_NAVIX_THROW_IF_NOT_MSG(optim.logfile, "could not open logfile");
        }
...
size_t PolysemousTraining::memory_usage_per_thread(
    ...
    FAISS_NAVIX_THROW_MSG("Invalid optmization type");
    return 0;
}
