...
#include <faiss_navix/impl/NNDescent.h>
...
void NNDescent::build(DistanceComputer& qdis, const int n, bool verbose) {
    FAISS_NAVIX_THROW_IF_NOT_MSG(L >= K, "L should be >= K in NNDescent.build");
    ...
    for (int i = 0; i < ntotal; i++) {
        ...
        for (int j = 0; j < K; j++) {
            FAISS_NAVIX_ASSERT(graph[i].pool[j].id < ntotal);
...
void NNDescent::search(
    ...
    FAISS_NAVIX_THROW_IF_NOT_MSG(has_built, "The index is not build yet.");
