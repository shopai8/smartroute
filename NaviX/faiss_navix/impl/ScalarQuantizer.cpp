...
#include <faiss_navix/utils/bf16.h>
#include <faiss_navix/utils/fp16.h>
...
struct Codec8bit {
    static FAISS_NAVIX_ALWAYS_INLINE void encode_component(
            float x,
            uint8_t* code,
            int i) {
        code[i] = (int)(255 * x);
    ...
    static FAISS_NAVIX_ALWAYS_INLINE float decode_component(
            const uint8_t* code,
            int i) {
    ...
    static FAISS_NAVIX_ALWAYS_INLINE __m512
...
#elif defined(__AVX2__)
    static FAISS_NAVIX_ALWAYS_INLINE __m256
...
#ifdef USE_NEON
    static FAISS_NAVIX_ALWAYS_INLINE float32x4x2_t
    decode_8_components(const uint8_t* code, int i) {
        float32_t result[8] = {};
...
struct Codec6bit {
    ...
    static FAISS_NAVIX_ALWAYS_INLINE __m256i load6(const uint16_t* code16) {
...
struct QuantizerTemplate<Codec, QuantizerTemplateScaling::UNIFORM, 1>
    ...
    FAISS_NAVIX_ALWAYS_INLINE float reconstruct_component(const uint8_t* code, int i)
            const {
        float xi = Codec::decode_component(code, i);
...
struct QuantizerTemplate<Codec, QuantizerTemplateScaling::UNIFORM, 16>
    ...
    FAISS_NAVIX_ALWAYS_INLINE __m512
...
struct QuantizerTemplate<Codec, QuantizerTemplateScaling::UNIFORM, 8>
    ...
    FAISS_NAVIX_ALWAYS_INLINE __m256
...
struct QuantizerTemplate<Codec, QuantizerTemplateScaling::UNIFORM, 8>
    ...
    FAISS_NAVIX_ALWAYS_INLINE float32x4x2_t
    reconstruct_8_components(const uint8_t* code, int i) const {
...
ScalarQuantizer::SQuantizer* select_quantizer_1(
    ...
    FAISS_NAVIX_THROW_MSG("unknown qtype");
}
...
void train_Uniform(
    ...
    if (rs == ScalarQuantizer::RS_minmax) {
        ...
        FAISS_NAVIX_THROW_MSG("Invalid qtype");
    }
    vmax -= vmin;
}
...
struct SimilarityL2<1> {
    ...
    FAISS_NAVIX_ALWAYS_INLINE void begin() {
        accu = 0;
        yi = y;
    }

    FAISS_NAVIX_ALWAYS_INLINE void add_component(float x) {
        float tmp = *yi++ - x;
        accu += tmp * tmp;
    }

    FAISS_NAVIX_ALWAYS_INLINE void add_component_2(float x1, float x2) {
        float tmp = x1 - x2;
        accu += tmp * tmp;
    }

    FAISS_NAVIX_ALWAYS_INLINE float result() {
        return accu;
    }
};
...
struct SimilarityL2<16> {
    ...
    FAISS_NAVIX_ALWAYS_INLINE void begin_16() {
        accu16 = _mm512_setzero_ps();
        yi = y;
    }

    FAISS_NAVIX_ALWAYS_INLINE void add_16_components(__m512 x) {
    ...
    FAISS_NAVIX_ALWAYS_INLINE void add_16_components_2(__m512 x, __m512 y_2) {
        __m512 tmp = _mm512_sub_ps(y_2, x);
        accu16 = _mm512_fmadd_ps(tmp, tmp, accu16);
    }

    FAISS_NAVIX_ALWAYS_INLINE float result_16() {
...
struct SimilarityL2<8> {
    ...
    FAISS_NAVIX_ALWAYS_INLINE void begin_8() {
        accu8 = _mm256_setzero_ps();
        yi = y;
    }

    FAISS_NAVIX_ALWAYS_INLINE void add_8_components(__m256 x) {
    ...
    FAISS_NAVIX_ALWAYS_INLINE void add_8_components_2(__m256 x, __m256 y_2) {
        __m256 tmp = _mm256_sub_ps(y_2, x);
        accu8 = _mm256_fmadd_ps(tmp, tmp, accu8);
    }

    FAISS_NAVIX_ALWAYS_INLINE float result_8() {
...
struct SimilarityL2<8> {
    ...
    FAISS_NAVIX_ALWAYS_INLINE void add_8_components(float32x4x2_t x) {
    ...
    FAISS_NAVIX_ALWAYS_INLINE void add_8_components_2(
            float32x4x2_t x,
            float32x4x2_t y) {
...
struct SimilarityIP<16> {
    ...
    FAISS_NAVIX_ALWAYS_INLINE void add_16_components_2(__m512 x1, __m512 x2) {
...
struct SimilarityIP<8> {
    ...
    FAISS_NAVIX_ALWAYS_INLINE void add_8_components_2(__m256 x1, __m256 x2) {
...
SQDistanceComputer* ScalarQuantizer::get_distance_computer(
    ...
    FAISS_NAVIX_THROW_IF_NOT(metric == METRIC_L2 || metric == METRIC_INNER_PRODUCT);
#if defined(USE_AVX512_F16C)
    if (d % 16 == 0) {
        if (metric == METRIC_L2) {
...
InvertedListScanner* sel3_InvertedListScanner(
    ...
    if (DCClass::Sim::metric_type == METRIC_L2) {
        ...
        FAISS_NAVIX_THROW_MSG("unsupported metric type");
    }
}
