...
void write_VectorTransform(const VectorTransform* vt, IOWriter* f) {
    if (const LinearTransform* lt = dynamic_cast<const LinearTransform*>(vt)) {
        ...
        FAISS_NAVIX_THROW_MSG("cannot serialize this");
    }
    // common fields
    WRITE1(vt->d_in);
    WRITE1(vt->d_out);
...
static void write_NSG(const NSG* nsg, IOWriter* f) {
    ...
    int K = graph->K;
    int N = graph->N;
    FAISS_NAVIX_THROW_IF_NOT(N == nsg->ntotal);
    FAISS_NAVIX_THROW_IF_NOT(K == nsg->R);
    FAISS_NAVIX_THROW_IF_NOT(true == graph->own_fields);
...
void write_index(const Index* idx, IOWriter* f, int io_flags) {
    if (idx == nullptr) {
        ...
        FAISS_NAVIX_THROW_IF_NOT(idxlsqfs || idxrqfs || idxplsqfs || idxprqfs);
        ...
        auto ivprqfs =
                dynamic_cast<const IndexIVFProductResidualQuantizerFastScan*>(
                        idx);
        FAISS_NAVIX_THROW_IF_NOT(ivlsqfs || ivrqfs || ivplsqfs || ivprqfs);
        ...
        FAISS_NAVIX_THROW_IF_NOT(idxnndflat != nullptr);
        ...
        FAISS_NAVIX_THROW_MSG("don't know how to serialize this type of index");
    }
}
