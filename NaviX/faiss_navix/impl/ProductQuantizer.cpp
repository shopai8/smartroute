...
void ProductQuantizer::set_derived_values() {
    ...
    FAISS_NAVIX_THROW_IF_MSG(nbits > 24, "nbits larger than 24 is not practical.");
    ksub = 1 << nbits;
...
void ProductQuantizer::compute_code(const float* x, uint8_t* code) const {
    switch (nbits) {
        case 8:
            faiss_navix::compute_code<PQEncoder8>(*this, x, code);
            break;

        case 16:
            faiss_navix::compute_code<PQEncoder16>(*this, x, code);
            break;

        default:
            faiss_navix::compute_code<PQEncoderGeneric>(*this, x, code);
            break;
    }
}
...
void ProductQuantizer::decode(const uint8_t* code, float* x) const {
    switch (nbits) {
        case 8:
            faiss_navix::decode<PQDecoder8>(*this, code, x);
            break;

        case 16:
            faiss_navix::decode<PQDecoder16>(*this, code, x);
            break;

        default:
            faiss_navix::decode<PQDecoderGeneric>(*this, code, x);
            break;
    }
}
...
void ProductQuantizer::compute_codes_with_assign_index(
        ...
        uint8_t* codes,
        size_t n) {
    FAISS_NAVIX_THROW_IF_NOT(assign_index && assign_index->d == dsub);
...
void ProductQuantizer::search(
    ...
    FAISS_NAVIX_THROW_IF_NOT(nx == res->nh);
...
void ProductQuantizer::search_sdc(
        ...
        float_maxheap_array_t* res,
        bool init_finalize_heap) const {
    FAISS_NAVIX_THROW_IF_NOT(sdc_table.size() == M * ksub * ksub);
    FAISS_NAVIX_THROW_IF_NOT(nbits == 8);
    size_t k = res->k;
