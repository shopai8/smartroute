...
int IOReader::filedescriptor() {
    FAISS_NAVIX_THROW_MSG("IOReader does not support memory mapping");
}

int IOWriter::filedescriptor() {
    FAISS_NAVIX_THROW_MSG("IOWriter does not support memory mapping");
}
...
size_t BufferedIOWriter::operator()(
    ...
    while (size > 0) {
        ...
        do {
            ...
            FAISS_NAVIX_THROW_IF_NOT(written > 0);
            ofs_2 += written;
        } while (ofs_2 != bsz);
