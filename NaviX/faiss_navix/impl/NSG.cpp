...
#include <faiss_navix/impl/NSG.h>
...
void NSG::search(
    ...
    FAISS_NAVIX_THROW_IF_NOT(is_built);
    FAISS_NAVIX_THROW_IF_NOT(final_graph);
...
void NSG::build(
        ...
        bool verbose) {
    FAISS_NAVIX_THROW_IF_NOT(!is_built && ntotal == 0);
...
#pragma omp parallel for
    for (int i = 0; i < ntotal; i++) {
        for (int j = 0; j < R; j++) {
            ...
            FAISS_NAVIX_THROW_IF_NOT(id < ntotal && (id >= 0 || id == EMPTY_ID));
        }
    }
}
