...
#include <faiss_navix/impl/residual_quantizer_encode_steps.h>
...
void ResidualQuantizer::initialize_from(
        ...
        int skip_M) {
    FAISS_NAVIX_THROW_IF_NOT(M + skip_M <= other.M);
    FAISS_NAVIX_THROW_IF_NOT(skip_M >= 0);
    ...
    *this = other;
    ...
    if (codebooks.size() > 0) {
        FAISS_NAVIX_THROW_IF_NOT(codebooks.size() == other.total_codebook_size * d);
...
float ResidualQuantizer::retrain_AQ_codebook(size_t n, const float* x) {
    FAISS_NAVIX_THROW_IF_NOT_MSG(n >= total_codebook_size, "too few training points");
    ...
    { // solve least squares
        ...
        sgelsd_(&ni,
                ...
                iwork.data(),
                &info);
        FAISS_NAVIX_THROW_IF_NOT_FMT(info == 0, "SGELS returned info=%d", int(info));
        if (verbose) {
    ...
    for (size_t i = 0; i < total_codebook_size; i++) {
        for (size_t j = 0; j < d; j++) {
            ...
            FAISS_NAVIX_THROW_IF_NOT(std::isfinite(codebooks[i * d + j]));
        }
    }
...
void ResidualQuantizer::compute_codes_add_centroids(
    ...
    FAISS_NAVIX_THROW_IF_NOT_MSG(is_trained, "RQ is not trained yet.");
