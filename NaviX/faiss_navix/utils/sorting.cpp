...
#include <faiss_navix/utils/sorting.h>
...
void bucket_sort_ref(
    ...
    for (size_t i = 0; i < nval; i++) {
        FAISS_NAVIX_THROW_IF_NOT(vals[i] < vmax);
        lims[vals[i] + 1]++;
    }
    ...
    FAISS_NAVIX_THROW_IF_NOT(lims[vmax] == nval);
...
void bucket_sort_inplace_ref(
    ...
    for (size_t i = 0; i < nval; i++) {
        FAISS_NAVIX_THROW_IF_NOT(vals[i] < nbucket);
        lims[vals[i] + 1]++;
    }
    double t1 = getmillisecs();
    // compute cumulative sum
    ...
    FAISS_NAVIX_THROW_IF_NOT(lims[nbucket] == nval);
    double t2 = getmillisecs();
...
struct ToWrite {
    ...
    void bucket_sort() {
        FAISS_NAVIX_THROW_IF_NOT(buckets.size() == rows.size());
        lims.resize(nbucket + 1);
        ...
        FAISS_NAVIX_THROW_IF_NOT(lims[nbucket] == buckets.size());
...
#pragma omp parallel for reduction(+ : num_errors)
    ...
    FAISS_NAVIX_THROW_IF_NOT_MSG(num_errors == 0, "hashtable capacity exhausted");
}
