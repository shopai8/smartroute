...
#include <faiss_navix/utils/approx_topk_hamming/approx_topk_hamming.h>
...
void hammings_knn_hc(
        ...
        const faiss_navix::IDSelector* sel = nullptr) {
    size_t k = ha->k;
    if (init_heap)
        ha->heapify();
            ...
            switch (approx_topk_mode) {
                ...
                default: {
                    for (size_t j = j0; j < j1; j++, bs2_ += bytes_per_code) {
                        ...
                        dis = hc.hamming(bs2_);
                        if (dis < bh_val_[0]) {
                            faiss_navix::maxheap_replace_top<hamdis_t>(
...
void hammings_knn_mc(
        ...
        const faiss_navix::IDSelector* sel) {
...
void bitvec_shuffle(
    ...
    for (size_t i = 0; i < db; i++) {
        FAISS_NAVIX_THROW_IF_NOT(order[i] >= 0 && order[i] < da);
    }
    size_t lda = (da + 7) / 8;
    size_t ldb = (db + 7) / 8;
...
void hammings(
        ...
        size_t ncodes,
        hamdis_t* __restrict dis) {
    FAISS_NAVIX_THROW_IF_NOT(ncodes % 8 == 0);
    switch (ncodes) {
        case 8:
            faiss_navix::hammings<64>(C64(a), C64(b), na, nb, dis);
            return;
        case 16:
            faiss_navix::hammings<128>(C64(a), C64(b), na, nb, dis);
            return;
        case 32:
            faiss_navix::hammings<256>(C64(a), C64(b), na, nb, dis);
            return;
        case 64:
            faiss_navix::hammings<512>(C64(a), C64(b), na, nb, dis);
            return;
        default:
            faiss_navix::hammings(C64(a), C64(b), na, nb, ncodes * 8, dis);
            return;
    }
}
...
void hamming_count_thres(
    ...
    switch (ncodes) {
        case 8:
            faiss_navix::hamming_count_thres<64>(
                    C64(bs1), C64(bs2), n1, n2, ht, nptr);
            return;
        case 16:
            faiss_navix::hamming_count_thres<128>(
                    C64(bs1), C64(bs2), n1, n2, ht, nptr);
            return;
        case 32:
            faiss_navix::hamming_count_thres<256>(
                    C64(bs1), C64(bs2), n1, n2, ht, nptr);
            return;
        case 64:
            faiss_navix::hamming_count_thres<512>(
                    C64(bs1), C64(bs2), n1, n2, ht, nptr);
            return;
        default:
            FAISS_NAVIX_THROW_FMT("not implemented for %zu bits", ncodes);
    }
}
...
void crosshamming_count_thres(
    ...
    switch (ncodes) {
        case 8:
            faiss_navix::crosshamming_count_thres<64>(C64(dbs), n, ht, nptr);
            return;
        case 16:
            faiss_navix::crosshamming_count_thres<128>(C64(dbs), n, ht, nptr);
            return;
        case 32:
            faiss_navix::crosshamming_count_thres<256>(C64(dbs), n, ht, nptr);
            return;
        case 64:
            faiss_navix::crosshamming_count_thres<512>(C64(dbs), n, ht, nptr);
            return;
        default:
            FAISS_NAVIX_THROW_FMT("not implemented for %zu bits", ncodes);
    }
...
size_t match_hamming_thres(
    ...
    switch (ncodes) {
        case 8:
            return faiss_navix::match_hamming_thres<64>(
                    C64(bs1), C64(bs2), n1, n2, ht, idx, dis);
        case 16:
            return faiss_navix::match_hamming_thres<128>(
                    C64(bs1), C64(bs2), n1, n2, ht, idx, dis);
        case 32:
            return faiss_navix::match_hamming_thres<256>(
                    C64(bs1), C64(bs2), n1, n2, ht, idx, dis);
        case 64:
            return faiss_navix::match_hamming_thres<512>(
                    C64(bs1), C64(bs2), n1, n2, ht, idx, dis);
        default:
            FAISS_NAVIX_THROW_FMT("not implemented for %zu bits", ncodes);
            return 0;
...
void pack_bitstrings(
        ...
        uint8_t* packed,
        size_t code_size) {
    FAISS_NAVIX_THROW_IF_NOT(code_size >= (M * nbit + 7) / 8);
...
void pack_bitstrings(
    ...
    FAISS_NAVIX_THROW_IF_NOT(code_size >= (totbit + 7) / 8);
#pragma omp parallel for if (n > 1000)
    for (int64_t i = 0; i < n; i++) {
        const int32_t* in = unpacked + i * M;
        uint8_t* out = packed + i * code_size;
