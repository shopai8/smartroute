...
#include <faiss_navix/utils/transpose/transpose-avx512-inl.h>
#elif defined(__AVX2__)
#include <faiss_navix/utils/transpose/transpose-avx2-inl.h>
#endif
...
float fvec_inner_product(const float* x, const float* y, size_t d) {
    float res = 0.F;
    FAISS_NAVIX_PRAGMA_IMPRECISE_LOOP
    for (size_t i = 0; i != d; ++i) {
        res += x[i] * y[i];
    }
    return res;
}
FAISS_NAVIX_PRAGMA_IMPRECISE_FUNCTION_END
...
void compute_PQ_dis_tables_dsub2(
        ...
        float* dis_tables) {
    size_t M = d / 2;
    FAISS_NAVIX_THROW_IF_NOT(ksub % 8 == 0);
