...
size_t get_mem_usage_kb() {
    ...
    FAISS_NAVIX_THROW_IF_NOT_MSG(f, "cannot open proc status file");
    size_t sz = 0;
    for (;;) {
        char buf[256];
        if (!fgets(buf, 256, f))
...
void bincode_hist(size_t n, size_t nbits, const uint8_t* codes, int* hist) {
    FAISS_NAVIX_THROW_IF_NOT(nbits % 8 == 0);
    size_t d = nbits / 8;
...
void CombinerRangeKNN<T>::write_result(T* D_res, int64_t* I_res) {
    FAISS_NAVIX_THROW_IF_NOT(L_res);
    int64_t j = 0;
    for (int64_t i = 0; i < nq; i++) {
