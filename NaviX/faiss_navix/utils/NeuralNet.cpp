...
#include <faiss_navix/utils/NeuralNet.h>
...
Tensor2DTemplate<T>& Tensor2DTemplate<T>::operator+=(
    ...
    FAISS_NAVIX_THROW_IF_NOT(shape[0] == other.shape[0]);
    FAISS_NAVIX_THROW_IF_NOT(shape[1] == other.shape[1]);
...
Tensor2D Linear::operator()(const Tensor2D& x) const {
    FAISS_NAVIX_THROW_IF_NOT(x.shape[1] == in_features);
    size_t n = x.shape[0];
    ...
    if (bias.size() > 0) {
        FAISS_NAVIX_THROW_IF_NOT(bias.size() == out_features);
        for (size_t i = 0; i < n; i++) {
...
Tensor2D Embedding::operator()(const Int32Tensor2D& code) const {
    FAISS_NAVIX_THROW_IF_NOT(code.shape[1] == 1);
    size_t n = code.shape[0];
    Tensor2D output(n, embedding_dim);
    for (size_t i = 0; i < n; ++i) {
        size_t ci = code.v[i];
        FAISS_NAVIX_THROW_IF_NOT(ci < num_embeddings);
...
Tensor2D concatenate_rows(const Tensor2D& x, const Tensor2D& y) {
    ...
    FAISS_NAVIX_THROW_IF_NOT(n == y.shape[0]);
...
nn::Tensor2D QINCoStep::decode(
    ...
    FAISS_NAVIX_THROW_IF_NOT(n == codes.shape[0]);
...
nn::Tensor2D QINCo::decode(const nn::Int32Tensor2D& codes) const {
    FAISS_NAVIX_THROW_IF_NOT(codes.shape[1] == M);
...
nn::Int32Tensor2D QINCo::encode(const nn::Tensor2D& x) const {
    FAISS_NAVIX_THROW_IF_NOT(x.shape[1] == d);
    size_t n = x.shape[0];
