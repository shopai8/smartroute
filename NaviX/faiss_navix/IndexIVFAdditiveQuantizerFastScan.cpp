...
void IndexIVFAdditiveQuantizerFastScan::init(
        ...
        MetricType metric,
        int bbs) {
    FAISS_NAVIX_THROW_IF_NOT(aq != nullptr);
    FAISS_NAVIX_THROW_IF_NOT(!aq->nbits.empty());
    FAISS_NAVIX_THROW_IF_NOT(aq->nbits[0] == 4);
...
void IndexIVFAdditiveQuantizerFastScan::estimate_norm_scale(
        idx_t n,
        const float* x_in) {
    FAISS_NAVIX_THROW_IF_NOT(metric_type == METRIC_L2);
...
#pragma omp for
    ...
    if (metric_type == METRIC_L2) {
        ...
        FAISS_NAVIX_THROW_IF_NOT(norm_tabs.size() == norm_dim12);
...
#pragma omp parallel for if (n > 100)
        ...
        FAISS_NAVIX_THROW_FMT("metric %d not supported", metric_type);
    }
}
...
IndexIVFLocalSearchQuantizerFastScan::IndexIVFLocalSearchQuantizerFastScan(
    ...
    FAISS_NAVIX_THROW_IF_NOT(nbits == 4);
