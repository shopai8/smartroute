...
#include <cublas_v2.h>
#include <faiss_navix/gpu/utils/DeviceTensor.cuh>
#include <faiss_navix/gpu/utils/Float16.cuh>
#include <faiss_navix/gpu/utils/HostTensor.cuh>
...
void runMatrixMult(
    ...
    FAISS_NAVIX_ASSERT(c.getSize(0) <= std::numeric_limits<int>::max());
    FAISS_NAVIX_ASSERT(c.getSize(1) <= std::numeric_limits<int>::max());

    FAISS_NAVIX_ASSERT(b.getSize(0) <= std::numeric_limits<int>::max());
    FAISS_NAVIX_ASSERT(b.getSize(1) <= std::numeric_limits<int>::max());

    FAISS_NAVIX_ASSERT(a.getSize(0) <= std::numeric_limits<int>::max());
    FAISS_NAVIX_ASSERT(a.getSize(1) <= std::numeric_limits<int>::max());
    ...
    FAISS_NAVIX_ASSERT(aM == cM);
    FAISS_NAVIX_ASSERT(aK == bK);
    FAISS_NAVIX_ASSERT(bN == cN);

    FAISS_NAVIX_ASSERT(a.getStride(1) == 1);
    FAISS_NAVIX_ASSERT(b.getStride(1) == 1);
    FAISS_NAVIX_ASSERT(c.getStride(1) == 1);
...
void runBatchMatrixMult(
    ...
    FAISS_NAVIX_ASSERT(c.getSize(0) <= std::numeric_limits<int>::max());
    FAISS_NAVIX_ASSERT(c.getSize(1) <= std::numeric_limits<int>::max());
    FAISS_NAVIX_ASSERT(c.getSize(2) <= std::numeric_limits<int>::max());

    FAISS_NAVIX_ASSERT(a.getSize(0) <= std::numeric_limits<int>::max());
    FAISS_NAVIX_ASSERT(a.getSize(1) <= std::numeric_limits<int>::max());
    FAISS_NAVIX_ASSERT(a.getSize(2) <= std::numeric_limits<int>::max());

    FAISS_NAVIX_ASSERT(a.getSize(0) <= std::numeric_limits<int>::max());
    FAISS_NAVIX_ASSERT(a.getSize(1) <= std::numeric_limits<int>::max());
    FAISS_NAVIX_ASSERT(a.getSize(2) <= std::numeric_limits<int>::max());

    FAISS_NAVIX_ASSERT(c.getSize(0) == a.getSize(0));
    FAISS_NAVIX_ASSERT(a.getSize(0) == b.getSize(0));

    // This uses the strided batch MM, which assumes a uniform stride
    FAISS_NAVIX_ASSERT(a.getStride(0) == a.getSize(1) * a.getSize(2));
    FAISS_NAVIX_ASSERT(b.getStride(0) == b.getSize(1) * b.getSize(2));
    FAISS_NAVIX_ASSERT(c.getStride(0) == c.getSize(1) * c.getSize(2));
