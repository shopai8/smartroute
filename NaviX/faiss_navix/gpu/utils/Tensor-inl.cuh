...
#include <faiss_navix/gpu/GpuFaissAssert.h>
...
__host__ __device__ Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::Tensor(
        ...
        : data_(data) {
    GPU_FAISS_NAVIX_ASSERT(sizes.size() == Dim);
    static_assert(Dim > 0, "must have > 0 dimensions");
...
__host__ void Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::copyFrom(
    ...
    GPU_FAISS_NAVIX_ASSERT(this->isContiguous());

    // Size must be the same (since dimensions are checked and
    // continuity is assumed, we need only check total number of
    // elements
    GPU_FAISS_NAVIX_ASSERT(this->numElements() == t.numElements());

    if (t.numElements() > 0) {
        GPU_FAISS_NAVIX_ASSERT(this->data_);
        GPU_FAISS_NAVIX_ASSERT(t.data());
...
__host__ void Tensor<T, Dim, InnerContig, IndexT, PtrTraits>::copyFrom(
    ...
    GPU_FAISS_NAVIX_ASSERT(this->numElements() == v.size());
...
__host__ __device__ Tensor<U, Dim, InnerContig, IndexT, PtrTraits> Tensor<
    ...
    GPU_FAISS_NAVIX_ASSERT(canCastResize<U>());
...
__host__ Tensor<T, Dim, InnerContig, NewIndexT, PtrTraits> Tensor<
    ...
    if (sizeof(NewIndexT) < sizeof(IndexT)) {
        GPU_FAISS_NAVIX_ASSERT(this->canUseIndexType<NewIndexT>());
    }
...
__host__ __device__ Tensor<T, Dim, InnerContig, IndexT, PtrTraits> Tensor<
    ...
    GPU_FAISS_NAVIX_ASSERT(dim1 >= 0 && dim1 < Dim);
    GPU_FAISS_NAVIX_ASSERT(dim2 >= 0 && dim2 < Dim);

    // If a tensor is innermost contiguous, one cannot transpose the innermost
    // dimension
    if (InnerContig) {
        GPU_FAISS_NAVIX_ASSERT(dim1 != Dim - 1 && dim2 != Dim - 1);
    }
...
__host__ __device__ Tensor<T, NewDim, InnerContig, IndexT, PtrTraits> Tensor<
    ...
    for (int i = NewDim; i < Dim; ++i) {
        GPU_FAISS_NAVIX_ASSERT(isContiguousDim(i));
    }
...
__host__ __device__ Tensor<T, Dim, InnerContig, IndexT, PtrTraits> Tensor<
    ...
    for (int i = 0; i < Dim; ++i) {
        if (i == dim) {
            GPU_FAISS_NAVIX_ASSERT(start + size <= size_[dim]);
            newSize[i] = size;
        } else {
            newSize[i] = size_[i];
        }
...
__host__ __device__ Tensor<T, NewDim, InnerContig, IndexT, PtrTraits> Tensor<
    ...
    GPU_FAISS_NAVIX_ASSERT(sizes.size() == NewDim);
    ...
    GPU_FAISS_NAVIX_ASSERT(curSize == newSize);
