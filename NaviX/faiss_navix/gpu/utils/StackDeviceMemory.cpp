...
#include <faiss_navix/gpu/utils/StackDeviceMemory.h>
...
char* StackDeviceMemory::Stack::getAlloc(size_t size, cudaStream_t stream) {
    ...
    FAISS_NAVIX_ASSERT(size <= sizeRemaining);
...
void StackDeviceMemory::deallocMemory(
        ...
        size_t size,
        void* p) {
    FAISS_NAVIX_ASSERT(p);
    FAISS_NAVIX_ASSERT(device == device_);
