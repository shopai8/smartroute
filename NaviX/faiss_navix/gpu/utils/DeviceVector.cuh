...
class DeviceVector {
   public:
    ...
    std::vector<OutT> copyToHost(cudaStream_t stream) const {
        FAISS_NAVIX_ASSERT(num_ * sizeof(T) % sizeof(OutT) == 0);

        std::vector<OutT> out((num_ * sizeof(T)) / sizeof(OutT));

        if (num_ > 0) {
            FAISS_NAVIX_ASSERT(data());
    ...
    size_t reclaim(bool exact, cudaStream_t stream) {
        ...
        if (free > (capacity_ / 4)) {
            ...
            FAISS_NAVIX_ASSERT(newCapacity < oldCapacity);
   ...
   private:
    void realloc_(size_t newCapacity, cudaStream_t stream) {
        FAISS_NAVIX_ASSERT(num_ <= newCapacity);
