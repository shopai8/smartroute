...
#include <faiss_navix/gpu/utils/Limits.cuh>
...
#define BLOCK_SELECT_IMPL(TYPE, DIR, WARP_Q, THREAD_Q)                         \
    void runBlockSelect_##TYPE##_##DIR##_##WARP_Q##_(                          \
            ...
            int k,                                                             \
            cudaStream_t stream) {                                             \
        FAISS_NAVIX_ASSERT(in.getSize(0) == outK.getSize(0));                        \
        FAISS_NAVIX_ASSERT(in.getSize(0) == outV.getSize(0));                        \
        FAISS_NAVIX_ASSERT(outK.getSize(1) == k);                                    \
        FAISS_NAVIX_ASSERT(outV.getSize(1) == k);                                    \
                                                                               \
        ...
        auto block = dim3(kBlockSelectNumThreads);                             \
                                                                               \
        FAISS_NAVIX_ASSERT(k <= WARP_Q);                                             \
        FAISS_NAVIX_ASSERT(dir == DIR);                                              \
                                                                               \
    ...
    void runBlockSelectPair_##TYPE##_##DIR##_##WARP_Q##_(                      \
            ...
            int k,                                                             \
            cudaStream_t stream) {                                             \
        FAISS_NAVIX_ASSERT(inK.isSameSize(inV));                                     \
        FAISS_NAVIX_ASSERT(outK.isSameSize(outV));                                   \
                                                                               \
