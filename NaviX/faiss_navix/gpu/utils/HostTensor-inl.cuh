...
__host__ HostTensor<T, Dim, InnerContig, IndexT, PtrTraits>::~HostTensor() {
    if (state_ == AllocState::Owner) {
        FAISS_NAVIX_ASSERT(this->data_ != nullptr);
        delete[] this->data_;
        this->data_ = nullptr;
    }
}
...
__host__ HostTensor<T, Dim, InnerContig, IndexT, PtrTraits>::HostTensor(
    ...
    FAISS_NAVIX_ASSERT(t.isContiguous());
...
__host__ HostTensor<T, Dim, InnerContig, IndexT, PtrTraits>& HostTensor<
    ...
    FAISS_NAVIX_ASSERT(this->isContiguous());
...
__host__ T HostTensor<T, Dim, InnerContig, IndexT, PtrTraits>::maxDiff(
    ...
    FAISS_NAVIX_ASSERT(size == t.numElements());
    FAISS_NAVIX_ASSERT(size > 0);
