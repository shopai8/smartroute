...
StandardGpuResourcesImpl::~StandardGpuResourcesImpl() {
    ...
    for (auto& entry : blasHandles_) {
        ...
        FAISS_NAVIX_ASSERT(blasStatus == CUBLAS_STATUS_SUCCESS);
    }
...
void StandardGpuResourcesImpl::setPinnedMemory(size_t size) {
    ...
    FAISS_NAVIX_ASSERT(defaultStreams_.size() == 0);
    FAISS_NAVIX_ASSERT(!pinnedMemAlloc_);
...
void StandardGpuResourcesImpl::setDefaultStream(
    ...
    if (isInitialized(device)) {
        ...
        if (it != userDefaultStreams_.end()) {
            ...
            FAISS_NAVIX_ASSERT(defaultStreams_.count(device));
...
void StandardGpuResourcesImpl::initializeForDevice(int device) {
    ...
    FAISS_NAVIX_ASSERT(device < getNumDevices());
    ...
    if (defaultStreams_.empty() && pinnedMemSize_ > 0) {
        ...
        if (defaultStreams_.empty() && pinnedMemSize_ > 0) {
            try {
                ...
                FAISS_NAVIX_THROW_MSG("CUDA memory allocation error");
            }
    ...
    FAISS_NAVIX_ASSERT(allocs_.count(device) == 0);
    allocs_[device] = std::unordered_map<void*, AllocRequest>();

    FAISS_NAVIX_ASSERT(tempMemory_.count(device) == 0);
...
void* StandardGpuResourcesImpl::allocMemory(const AllocRequest& req) {
    ...
    if (adjReq.space == MemorySpace::Temporary) {
        ...
        if (err != cudaSuccess) {
            ...
            FAISS_NAVIX_THROW_IF_NOT_FMT(err == cudaSuccess, "%s", str.c_str());
        }
#endif
        ...
        FAISS_NAVIX_ASSERT_FMT(false, "unknown MemorySpace %d", (int)adjReq.space);
    }
...
void StandardGpuResourcesImpl::deallocMemory(int device, void* p) {
    FAISS_NAVIX_ASSERT(isInitialized(device));
    ...
    if (req.space == MemorySpace::Temporary) {
        ...
        FAISS_NAVIX_ASSERT_FMT(false, "unknown MemorySpace %d", (int)req.space);
    }
...
size_t StandardGpuResourcesImpl::getTempMemoryAvailable(int device) const {
    ...
    FAISS_NAVIX_ASSERT(it != tempMemory_.end());
