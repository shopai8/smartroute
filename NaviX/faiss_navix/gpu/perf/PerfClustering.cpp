...
#include <faiss_navix/gpu/perf/IndexWrapper.h>
#include <faiss_navix/gpu/utils/DeviceUtils.h>
#include <faiss_navix/gpu/utils/Timer.h>
...
int main(int argc, char** argv) {
    ...
    auto initFn = [](faiss_navix::gpu::GpuResourcesProvider* res,
                     int dev) -> std::unique_ptr<faiss_navix::gpu::GpuIndexFlat> {
        ...
        auto p = std::unique_ptr<faiss_navix::gpu::GpuIndexFlat>(
                FLAGS_L2_metric
                        ? (faiss_navix::gpu::GpuIndexFlat*)new faiss_navix::gpu::
    ...
    IndexWrapper<faiss_navix::gpu::GpuIndexFlat> gpuIndex(FLAGS_num_gpus, initFn);

    CUDA_VERIFY(cudaProfilerStart());
    faiss_navix::gpu::synchronizeAllDevices();
    ...
    auto tc = new faiss_navix::TimeoutCallback();
    faiss_navix::InterruptCallback::instance.reset(tc);

    faiss_navix::Clustering kmeans(FLAGS_dim, FLAGS_k, cp);
