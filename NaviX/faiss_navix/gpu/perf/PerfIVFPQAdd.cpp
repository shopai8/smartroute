...
int main(int argc, char** argv) {
    ...
    faiss_navix::IndexFlatL2 coarseQuantizer(dim);
    ...
    config.device = 0;
    config.indicesOptions = (faiss_navix::gpu::IndicesOptions)FLAGS_index;
    ...
    for (int i = 0; i < FLAGS_batches; ++i) {
        ...
        auto addVecs = faiss_navix::gpu::randVecs(FLAGS_batch_size, dim);

        if (FLAGS_time_gpu) {
            faiss_navix::gpu::CpuTimer timer;
