...
void fillAndSave(T& index, int numTrain, int num, int dim) {
    auto trainVecs = faiss_navix::gpu::randVecs(numTrain, dim);
    ...
    for (int i = 0; i < num; i += kAddChunk) {
        ...
        auto vecs = faiss_navix::gpu::randVecs(numRemaining, dim);

        printf("adding at %d: %d\n", i, numRemaining);
        index.add(numRemaining, vecs.data());
    }

    faiss_navix::write_index(&index, FLAGS_out.c_str());
}
...
int main(int argc, char** argv) {
    ...
    if (FLAGS_ivfpq) {
        ...
        faiss_navix::IndexFlatL2 quantizerL2(dim);
        faiss_navix::IndexFlatIP quantizerIP(dim);

        faiss_navix::IndexFlat* quantizer = FLAGS_l2
                ? (faiss_navix::IndexFlat*)&quantizerL2
                : (faiss_navix::IndexFlat*)&quantizerIP;

        faiss_navix::IndexIVFFlat index(
                quantizer,
                dim,
                numCentroids,
                FLAGS_l2 ? faiss_navix::METRIC_L2 : faiss_navix::METRIC_INNER_PRODUCT);
