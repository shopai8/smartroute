...
class GpuIndexBinaryFlat : public IndexBinary {
   public:
    /// Construct from a pre-existing faiss_navix::IndexBinaryFlat instance, copying
    /// data over to the given GPU
    GpuIndexBinaryFlat(
            GpuResourcesProvider* resources,
            const faiss_navix::IndexBinaryFlat* index,
    ...
    void copyFrom(const faiss_navix::IndexBinaryFlat* index);

    /// Copy ourselves to the given CPU index; will overwrite all data
    /// in the index instance
    void copyTo(faiss_navix::IndexBinaryFlat* index) const;

    void add(faiss_navix::idx_t n, const uint8_t* x) override;
    ...
    void search(
            idx_t n,
            const uint8_t* x,
            // faiss_navix::IndexBinary has idx_t for k
            idx_t k,
            int32_t* distances,
            faiss_navix::idx_t* labels,
            const faiss_navix::SearchParameters* params = nullptr) const override;

    void reconstruct(faiss_navix::idx_t key, uint8_t* recons) const override;
