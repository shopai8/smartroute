...
#include <faiss_navix/gpu/GpuIndexIVFPQ.h>
...
void pickEncoding(int& codes, int& dim) {
    ...
    while (true) {
        codes = codeSizes[faiss_navix::gpu::randVal(0, codeSizes.size() - 1)];
        dim = codes * dimSizes[faiss_navix::gpu::randVal(0, dimSizes.size() - 1)];
...
void pickCuvsEncoding(int& codes, int& dim, int bitsPerCode) {
    ...
    while (true) {
        codes = faiss_navix::gpu::randVal(0, 96);
        dim = codes * dimSizes[faiss_navix::gpu::randVal(0, dimSizes.size() - 1)];
...
struct Options {
    Options() {
        numAdd = faiss_navix::gpu::randVal(2000, 5000);
        numCentroids = std::sqrt((float)numAdd);
        numTrain = numCentroids * 40;

        pickEncoding(codes, dim);

        // TODO: Change back to `faiss_navix::gpu::randVal(3, 7)` when we
        // officially support non-multiple of 8 subcodes for IVFPQ.
        bitsPerCode = 8;

        nprobe = std::min(faiss_navix::gpu::randVal(40, 1000), numCentroids);
        numQuery = faiss_navix::gpu::randVal(4, 8);
        ...
        // only use a small k
        k = std::min(faiss_navix::gpu::randVal(5, 20), numAdd / 40);
        usePrecomputed = faiss_navix::gpu::randBool();
        ...
        if (codes > 48) {
            ...
            useFloat16 = faiss_navix::gpu::randBool();
        }
...
void queryTest(Options opt, faiss_navix::MetricType metricType) {
    ...
    faiss_navix::IndexFlatL2 coarseQuantizerL2(opt.dim);
    faiss_navix::IndexFlatIP coarseQuantizerIP(opt.dim);
    faiss_navix::Index* quantizer = metricType == faiss_navix::METRIC_L2
            ? (faiss_navix::Index*)&coarseQuantizerL2
            : (faiss_navix::Index*)&coarseQuantizerIP;

    faiss_navix::IndexIVFPQ cpuIndex(
    ...
    faiss_navix::gpu::GpuIndexIVFPQConfig config;
    ...
    faiss_navix::gpu::GpuIndexIVFPQ gpuIndex(&res, &cpuIndex, config);
...
TEST(TestGpuIndexIVFPQ, Query_L2) {
    for (int tries = 0; tries < 2; ++tries) {
        ...
        queryTest(opt, faiss_navix::MetricType::METRIC_L2);
    }
}
...
TEST(TestGpuIndexIVFPQ, Query_IP) {
    for (int tries = 0; tries < 2; ++tries) {
        Options opt;
        queryTest(opt, faiss_navix::MetricType::METRIC_INNER_PRODUCT);
    }
}
...
void testMMCodeDistance(faiss_navix::MetricType mt) {
    ...
    for (int tries = 0; tries < 2; ++tries) {
        ...
        faiss_navix::IndexFlat coarseQuantizer(opt.dim, mt);
        faiss_navix::IndexIVFPQ cpuIndex(
                &coarseQuantizer,
                opt.dim,
                opt.numCentroids,
...
TEST(TestGpuIndexIVFPQ, Query_L2_MMCodeDistance) {
    testMMCodeDistance(faiss_navix::MetricType::METRIC_L2);
}

TEST(TestGpuIndexIVFPQ, Query_IP_MMCodeDistance) {
    testMMCodeDistance(faiss_navix::MetricType::METRIC_INNER_PRODUCT);
}
...
TEST(TestGpuIndexIVFPQ, Float16Coarse) {
    ...
    faiss_navix::IndexFlatL2 coarseQuantizer(opt.dim);
    faiss_navix::IndexIVFPQ cpuIndex(
            &coarseQuantizer,
            opt.dim,
            opt.numCentroids,
...
void addTest(Options opt, faiss_navix::MetricType metricType) {
    std::vector<float> trainVecs = faiss_navix::gpu::randVecs(opt.numTrain, opt.dim);
    std::vector<float> addVecs = faiss_navix::gpu::randVecs(opt.numAdd, opt.dim);
...
TEST(TestGpuIndexIVFPQ, Add_L2) {
    for (int tries = 0; tries < 2; ++tries) {
        Options opt;
        addTest(opt, faiss_navix::METRIC_L2);
    }
}
...
TEST(TestGpuIndexIVFPQ, Add_IP) {
    for (int tries = 0; tries < 2; ++tries) {
        Options opt;
        addTest(opt, faiss_navix::METRIC_INNER_PRODUCT);
    }
}
...
void copyToTest(Options opt) {
    for (int tries = 0; tries < 2; ++tries) {
        ...
        faiss_navix::gpu::GpuIndexIVFPQ gpuIndex(
                &res,
                opt.dim,
                opt.numCentroids,
                opt.codes,
                opt.bitsPerCode,
                faiss_navix::METRIC_L2,
                config);
        gpuIndex.nprobe = opt.nprobe;
        ...
        faiss_navix::IndexIVFPQ cpuIndex(&cpuQuantizer, 1, 1, 1, 1);
...
void copyFromTest(Options opt) {
    ...
    // Use garbage values to see if we overwrite them
    faiss_navix::gpu::GpuIndexIVFPQ gpuIndex(
            &res, 1, 1, 1, 8, faiss_navix::METRIC_L2, config);
    gpuIndex.nprobe = 1;
...
TEST(TestGpuIndexIVFPQ, Query_L2_Cuvs) {
    for (int tries = 0; tries < 2; ++tries) {
        Options opt;
        opt.bitsPerCode = faiss_navix::gpu::randVal(4, 8);
        opt.useCuvs = true;
...
TEST(TestGpuIndexIVFPQ, UnifiedMemory) {
    ...
    faiss_navix::gpu::GpuIndexIVFPQ cuvsGpuIndex(
            &res,
            dim,
            numCentroids,
            codes,
