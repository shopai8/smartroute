...
#include <faiss_navix/gpu/GpuIndexIVFFlat.h>
...
void queryTest(
        Options opt,
        faiss_navix::MetricType metricType,
        bool useFloat16CoarseQuantizer) {
    for (int tries = 0; tries < 2; ++tries) {
        std::vector<float> trainVecs =
                faiss_navix::gpu::randVecs(opt.numTrain, opt.dim);
        std::vector<float> addVecs = faiss_navix::gpu::randVecs(opt.numAdd, opt.dim);

        faiss_navix::IndexFlatL2 quantizerL2(opt.dim);
        faiss_navix::IndexFlatIP quantizerIP(opt.dim);
        faiss_navix::Index* quantizer = metricType == faiss_navix::METRIC_L2
                ? (faiss_navix::Index*)&quantizerL2
                : (faiss_navix::Index*)&quantizerIP;

        faiss_navix::IndexIVFFlat cpuIndex(
                quantizer, opt.dim, opt.numCentroids, metricType);
        cpuIndex.train(opt.numTrain, trainVecs.data());
        cpuIndex.add(opt.numAdd, addVecs.data());
        cpuIndex.nprobe = opt.nprobe;

        faiss_navix::gpu::StandardGpuResources res;
        res.noTempMemory();

        faiss_navix::gpu::GpuIndexIVFFlatConfig config;
        config.device = opt.device;
        config.indicesOptions = opt.indicesOpt;
        config.flatConfig.useFloat16 = useFloat16CoarseQuantizer;
        config.use_cuvs = opt.useCuvs;

        faiss_navix::gpu::GpuIndexIVFFlat gpuIndex(
        ...
        faiss_navix::gpu::compareIndices(
                cpuIndex,
                gpuIndex,
                opt.numQuery,
                opt.dim,
...
void addTest(
    ...
    for (int tries = 0; tries < 2; ++tries) {
        ...
        config.indicesOptions =
                opt.useCuvs ? faiss_navix::gpu::INDICES_64_BIT : opt.indicesOpt;
        config.flatConfig.useFloat16 = useFloat16CoarseQuantizer;
        config.use_cuvs = opt.useCuvs;
...
void copyToTest(bool useFloat16CoarseQuantizer, bool useCuvs) {
    Options opt;
    std::vector<float> trainVecs = faiss_navix::gpu::randVecs(opt.numTrain, opt.dim);
    std::vector<float> addVecs = faiss_navix::gpu::randVecs(opt.numAdd, opt.dim);
    ...
    config.device = opt.device;
    config.indicesOptions =
            useCuvs ? faiss_navix::gpu::INDICES_64_BIT : opt.indicesOpt;
    config.flatConfig.useFloat16 = useFloat16CoarseQuantizer;
    config.use_cuvs = useCuvs;

    faiss_navix::gpu::GpuIndexIVFFlat gpuIndex(
            &res, opt.dim, opt.numCentroids, faiss_navix::METRIC_L2, config);
    gpuIndex.train(opt.numTrain, trainVecs.data());
    gpuIndex.add(opt.numAdd, addVecs.data());
    gpuIndex.nprobe = opt.nprobe;

    // use garbage values to see if we overwrite then
    faiss_navix::IndexFlatL2 cpuQuantizer(1);
    faiss_navix::IndexIVFFlat cpuIndex(&cpuQuantizer, 1, 1, faiss_navix::METRIC_L2);
    cpuIndex.nprobe = 1;
...
void copyFromTest(bool useFloat16CoarseQuantizer, bool useCuvs) {
    ...
    faiss_navix::IndexFlatL2 cpuQuantizer(opt.dim);
    faiss_navix::IndexIVFFlat cpuIndex(
            &cpuQuantizer, opt.dim, opt.numCentroids, faiss_navix::METRIC_L2);
    cpuIndex.nprobe = opt.nprobe;
    cpuIndex.train(opt.numTrain, trainVecs.data());
    cpuIndex.add(opt.numAdd, addVecs.data());
    ...
    faiss_navix::gpu::GpuIndexIVFFlat gpuIndex(&res, 1, 1, faiss_navix::METRIC_L2, config);
    gpuIndex.nprobe = 1;
...
TEST(TestGpuIndexIVFFlat, Float32_32_Add_L2) {
    addTest(faiss_navix::METRIC_L2, false, false);

#if defined USE_NVIDIA_CUVS
    addTest(faiss_navix::METRIC_L2, false, true);
#endif
}

TEST(TestGpuIndexIVFFlat, Float32_32_Add_IP) {
    addTest(faiss_navix::METRIC_INNER_PRODUCT, false, false);

#if defined USE_NVIDIA_CUVS
    addTest(faiss_navix::METRIC_INNER_PRODUCT, false, true);
#endif
}

TEST(TestGpuIndexIVFFlat, Float16_32_Add_L2) {
    addTest(faiss_navix::METRIC_L2, true, false);

#if defined USE_NVIDIA_CUVS
    addTest(faiss_navix::METRIC_L2, true, true);
#endif
}

TEST(TestGpuIndexIVFFlat, Float16_32_Add_IP) {
    addTest(faiss_navix::METRIC_INNER_PRODUCT, true, false);

#if defined USE_NVIDIA_CUVS
    addTest(faiss_navix::METRIC_INNER_PRODUCT, true, true);
#endif
}
...
TEST(TestGpuIndexIVFFlat, Float32_Query_L2) {
    Options opt;
    queryTest(opt, faiss_navix::METRIC_L2, false);

#if defined USE_NVIDIA_CUVS
    opt.useCuvs = true;
    opt.indicesOpt = faiss_navix::gpu::INDICES_64_BIT;
    queryTest(opt, faiss_navix::METRIC_L2, false);
#endif
}
...
TEST(TestGpuIndexIVFFlat, Float32_Query_IP) {
    Options opt;
    queryTest(opt, faiss_navix::METRIC_INNER_PRODUCT, false);
...
TEST(TestGpuIndexIVFFlat, Float16_32_Query_L2) {
    Options opt;
    queryTest(opt, faiss_navix::METRIC_L2, true);
...
TEST(TestGpuIndexIVFFlat, Float16_32_Query_IP) {
    Options opt;
    queryTest(opt, faiss_navix::METRIC_INNER_PRODUCT, true);
...
TEST(TestGpuIndexIVFFlat, Float32_negative) {
    ...
    auto trainVecs = faiss_navix::gpu::randVecs(opt.numTrain, opt.dim);
    auto addVecs = faiss_navix::gpu::randVecs(opt.numAdd, opt.dim);
    ...
    faiss_navix::IndexFlatIP quantizerIP(opt.dim);
    faiss_navix::Index* quantizer = (faiss_navix::Index*)&quantizerIP;

    faiss_navix::IndexIVFFlat cpuIndex(
            quantizer, opt.dim, opt.numCentroids, faiss_navix::METRIC_INNER_PRODUCT);
    cpuIndex.train(opt.numTrain, trainVecs.data());
    cpuIndex.add(opt.numAdd, addVecs.data());
    cpuIndex.nprobe = opt.nprobe;
    ...
    auto queryVecs = faiss_navix::gpu::randVecs(opt.numQuery, opt.dim);
    ...
    config.indicesOptions = faiss_navix::gpu::INDICES_64_BIT;

    faiss_navix::gpu::GpuIndexIVFFlat cuvsGpuIndex(
            &res, cpuIndex.d, cpuIndex.nlist, cpuIndex.metric_type, config);
...
TEST(TestGpuIndexIVFFlat, QueryNaN) {
    ...
    std::vector<faiss_navix::idx_t> indices(numQuery * opt.k, 0);

    faiss_navix::gpu::GpuIndexIVFFlatConfig config;
    config.device = opt.device;
    config.indicesOptions = opt.indicesOpt;
    config.flatConfig.useFloat16 = faiss_navix::gpu::randBool();
    config.use_cuvs = false;
...
TEST(TestGpuIndexIVFFlat, AddNaN) {
    ...
    std::vector<float> queryVecs = faiss_navix::gpu::randVecs(opt.numQuery, opt.dim);
    std::vector<float> distance(opt.numQuery * opt.k, 0);
    std::vector<faiss_navix::idx_t> indices(opt.numQuery * opt.k, 0);
...
TEST(TestGpuIndexIVFFlat, UnifiedMemory) {
    ...
    // multiple devices
    int device = faiss_navix::gpu::randVal(0, faiss_navix::gpu::getNumDevices() - 1);

    if (!faiss_navix::gpu::getFullUnifiedMemSupport(device)) {
        return;
    }
    ...
    std::vector<float> trainVecs = faiss_navix::gpu::randVecs(numTrain, dim);
    std::vector<float> addVecs = faiss_navix::gpu::randVecs(numAdd, dim);

    faiss_navix::IndexFlatL2 quantizer(dim);
    faiss_navix::IndexIVFFlat cpuIndex(
            &quantizer, dim, numCentroids, faiss_navix::METRIC_L2);
    ...
    config.memorySpace = faiss_navix::gpu::MemorySpace::Unified;
    config.use_cuvs = false;

    faiss_navix::gpu::GpuIndexIVFFlat gpuIndex(
            &res, dim, numCentroids, faiss_navix::METRIC_L2, config);
    gpuIndex.copyFrom(&cpuIndex);
...
TEST(TestGpuIndexIVFFlat, LongIVFList) {
    ...
    if (faiss_navix::gpu::getFreeMemory(device) < kMem) {
...
TEST(TestGpuIndexIVFFlat, Reconstruct_n) {
    ...
    config.indicesOptions = faiss_navix::gpu::INDICES_32_BIT;

    faiss_navix::gpu::GpuIndexIVFFlat gpuIndex1(
            &res, opt.dim, opt.numCentroids, faiss_navix::METRIC_L2, config);
    ...
    config.indicesOptions = faiss_navix::gpu::INDICES_CPU;

    faiss_navix::gpu::GpuIndexIVFFlat gpuIndex2(
            &res, opt.dim, opt.numCentroids, faiss_navix::METRIC_L2, config);
