...
float eval_codec(faiss_navix::ResidualQuantizer* q, int nb, float* xb) {
    // Compute codes
...
TEST(TestGpuResidualQuantizer, TestNcall) {
    ...
    std::vector<float> xt = faiss_navix::gpu::randVecs(nt, d);
    std::vector<float> xb = faiss_navix::gpu::randVecs(nb, d);
    faiss_navix::ResidualQuantizer rq0(d, 4, 6);
    rq0.train(nt, xt.data());
    float err_rq0 = eval_codec(&rq0, nb, xb.data());
    faiss_navix::ResidualQuantizer rq1(d, 4, 6);
    faiss_navix::gpu::GpuProgressiveDimIndexFactory fac(1);
