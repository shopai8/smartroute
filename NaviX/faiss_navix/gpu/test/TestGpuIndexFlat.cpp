...
#include <faiss_navix/gpu/impl/IndexUtils.h>
...
struct TestFlatOptions {
    TestFlatOptions()
            : metric(faiss_navix::MetricType::METRIC_L2),
              metricArg(0),
              useFloat16(false),
              numVecsOverride(-1),
...
void testFlat(const TestFlatOptions& opt) {
    ...
    int k = opt.useFloat16
            ? std::min(faiss_navix::gpu::randVal(1, 50), numVecs)
            : std::min(
                      faiss_navix::gpu::randVal(1, faiss_navix::gpu::getMaxKSelection()),
                      numVecs);
    if (opt.kOverride > 0) {
        k = opt.kOverride;
    }

    faiss_navix::IndexFlat cpuIndex(dim, opt.metric);
    ...
    faiss_navix::gpu::GpuIndexFlatConfig config;
    config.device = device;
    config.useFloat16 = opt.useFloat16;
    config.use_cuvs = opt.use_cuvs;

    faiss_navix::gpu::GpuIndexFlat gpuIndex(&res, dim, opt.metric, config);
    gpuIndex.metric_arg = opt.metricArg;

    std::vector<float> vecs = faiss_navix::gpu::randVecs(numVecs, dim);
...
TEST(TestGpuIndexFlat, IP_Float32) {
    for (int tries = 0; tries < 3; ++tries) {
        TestFlatOptions opt;
        opt.metric = faiss_navix::MetricType::METRIC_INNER_PRODUCT;
...
TEST(TestGpuIndexFlat, L1_Float32) {
    TestFlatOptions opt;
    opt.metric = faiss_navix::MetricType::METRIC_L1;
    opt.useFloat16 = false;
...
TEST(TestGpuIndexFlat, Lp_Float32) {
    TestFlatOptions opt;
    opt.metric = faiss_navix::MetricType::METRIC_Lp;
    opt.metricArg = 5;
    opt.useFloat16 = false;
...
TEST(TestGpuIndexFlat, L2_Float32) {
    for (int tries = 0; tries < 3; ++tries) {
        TestFlatOptions opt;
        opt.metric = faiss_navix::MetricType::METRIC_L2;
...
TEST(TestGpuIndexFlat, L2_k_2048) {
    if (faiss_navix::gpu::getMaxKSelection() >= 2048) {
        TestFlatOptions opt;
        opt.metric = faiss_navix::MetricType::METRIC_L2;
        opt.useFloat16 = false;
        opt.kOverride = 2048;
...
TEST(TestGpuIndexFlat, QueryEmpty) {
    ...
    int dim = 128;
    faiss_navix::gpu::GpuIndexFlatL2 gpuIndex(&res, dim, config);
    ...
    std::vector<faiss_navix::idx_t> ind(numQuery * k);
...
void testCopyFrom(bool use_cuvs) {
    int numVecs = faiss_navix::gpu::randVal(100, 200);
    int dim = faiss_navix::gpu::randVal(1, 1000);

    std::vector<float> vecs = faiss_navix::gpu::randVecs(numVecs, dim);

    faiss_navix::IndexFlatL2 cpuIndex(dim);
    cpuIndex.add(numVecs, vecs.data());
    ...
    for (bool useFloat16 : {false, true}) {
        ...
        faiss_navix::gpu::GpuIndexFlatL2 gpuIndex(&res, 2000, config);
        ...
        if (useFloat16) {
            EXPECT_EQ(gpuVals, faiss_navix::gpu::roundToHalf(cpuVals));
        } else {
...
void testCopyTo(bool use_cuvs) {
    ...
    for (bool useFloat16 : {false, true}) {
        ...
        // Fill with garbage values
        faiss_navix::IndexFlatL2 cpuIndex(2000);
...
void testUnifiedMemory(bool use_cuvs) {
    ...
    faiss_navix::IndexFlatL2 cpuIndexL2(dim);
    ...
    faiss_navix::gpu::GpuIndexFlatL2 gpuIndexL2(&res, dim, config);
...
void testLargeIndex(bool use_cuvs) {
    ...
    auto xb = faiss_navix::gpu::randVecs(nb, dim);
...
void testResidual(bool use_cuvs) {
    ...
    int dim = 32;
    faiss_navix::IndexFlat cpuIndex(dim, faiss_navix::MetricType::METRIC_L2);
    faiss_navix::gpu::GpuIndexFlat gpuIndex(
            &res, dim, faiss_navix::MetricType::METRIC_L2, config);

    int numVecs = 100;
    auto vecs = faiss_navix::gpu::randVecs(numVecs, dim);
    cpuIndex.add(numVecs, vecs.data());
    gpuIndex.add(numVecs, vecs.data());

    auto indexVecs = std::vector<faiss_navix::idx_t>{0, 2, 4, 6, 8};
    auto queryVecs = faiss_navix::gpu::randVecs(indexVecs.size(), dim);
...
void testReconstruct(bool use_cuvs) {
    ...
    int numVecs = 100;
    auto vecs = faiss_navix::gpu::randVecs(numVecs, dim);
    auto vecs16 = faiss_navix::gpu::roundToHalf(vecs);
    ...
    for (bool useFloat16 : {false, true}) {
        ...
        if (false) {
            auto reconstructKeys = std::vector<faiss_navix::idx_t>{1, 3, 5};
...
void testSearchAndReconstruct(bool use_cuvs) {
    ...
    auto xb = faiss_navix::gpu::randVecs(nb, dim);
    auto xq = faiss_navix::gpu::randVecs(nq, dim);
    ...
    std::vector<faiss_navix::idx_t> refIndices(nq * k, -1);
    ...
    std::vector<faiss_navix::idx_t> testIndices(nq * k, -1);
    ...
    for (int i = 0; i < nq; ++i) {
        std::unordered_map<faiss_navix::idx_t, int> refLocation;
