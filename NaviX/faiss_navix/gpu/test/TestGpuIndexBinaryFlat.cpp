...
void compareBinaryDist(
        ...
        const std::vector<faiss_navix::idx_t>& cpuLabels,
        const std::vector<int>& gpuDist,
        const std::vector<faiss_navix::idx_t>& gpuLabels,
        int numQuery,
        int k) {
    for (int i = 0; i < numQuery; ++i) {
        ...
        // others we can check.
        std::set<faiss_navix::idx_t> cpuLabelSet;
        std::set<faiss_navix::idx_t> gpuLabelSet;
...
void testGpuIndexBinaryFlat(int kOverride = -1) {
    ...
    faiss_navix::gpu::GpuIndexBinaryFlatConfig config;
    config.device = faiss_navix::gpu::randVal(0, faiss_navix::gpu::getNumDevices() - 1);

    // multiples of 8 and multiples of 32 use different implementations
    int dims = faiss_navix::gpu::randVal(1, 20) * DimMultiple;
    faiss_navix::gpu::GpuIndexBinaryFlat gpuIndex(&res, dims, config);

    faiss_navix::IndexBinaryFlat cpuIndex(dims);

    int k = kOverride > 0
            ? kOverride
            : faiss_navix::gpu::randVal(1, faiss_navix::gpu::getMaxKSelection());
    int numVecs = faiss_navix::gpu::randVal(k + 1, 20000);
    int numQuery = faiss_navix::gpu::randVal(1, 1000);

    auto data = faiss_navix::gpu::randBinaryVecs(numVecs, dims);
    gpuIndex.add(numVecs, data.data());
    cpuIndex.add(numVecs, data.data());

    auto query = faiss_navix::gpu::randBinaryVecs(numQuery, dims);

    std::vector<int> cpuDist(numQuery * k);
    std::vector<faiss_navix::idx_t> cpuLabels(numQuery * k);

    cpuIndex.search(
            numQuery, query.data(), k, cpuDist.data(), cpuLabels.data());

    std::vector<int> gpuDist(numQuery * k);
    std::vector<faiss_navix::idx_t> gpuLabels(numQuery * k);
...
TEST(TestGpuIndexBinaryFlat, LargeIndex) {
    ...
    auto xb = faiss_navix::gpu::randBinaryVecs(nb, dims);
    auto xq = faiss_navix::gpu::randBinaryVecs(nq, dims);
    gpuIndex.add(nb, xb.data());
    cpuIndex.add(nb, xb.data());

    std::vector<int> cpuDist(nq * k);
    std::vector<faiss_navix::idx_t> cpuLabels(nq * k);

    cpuIndex.search(nq, xq.data(), k, cpuDist.data(), cpuLabels.data());

    std::vector<int> gpuDist(nq * k);
    std::vector<faiss_navix::idx_t> gpuLabels(nq * k);
...
TEST(TestGpuIndexBinaryFlat, Reconstruct) {
    ...
    faiss_navix::byte_rand(xb.data(), xb.size(), 123);
    std::unique_ptr<faiss_navix::IndexBinaryFlat> index(
            new faiss_navix::IndexBinaryFlat(64));
    index->add(n, xb.data());
    ...
    std::unique_ptr<faiss_navix::gpu::GpuIndexBinaryFlat> index2(
            new faiss_navix::gpu::GpuIndexBinaryFlat(&res, index.get()));
