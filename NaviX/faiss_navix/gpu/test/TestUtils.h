...
T randSelect(std::initializer_list<T> vals) {
    FAISS_NAVIX_ASSERT(vals.size() > 0);
...
void testIVFEquality(A& cpuIndex, B& gpuIndex) {
    ...
    for (int i = 0; i < cpuIndex.nlist; ++i) {
        ...
        auto sc = faiss_navix::InvertedLists::ScopedCodes(cpuLists, i);
        std::memcpy(
                cpuCodes.data(),
                sc.get(),
                cpuLists->list_size(i) * cpuLists->code_size);
        ...
        auto si = faiss_navix::InvertedLists::ScopedIds(cpuLists, i);
        std::memcpy(
                cpuIndices.data(),
                si.get(),
                cpuLists->list_size(i) * sizeof(faiss_navix::idx_t));
