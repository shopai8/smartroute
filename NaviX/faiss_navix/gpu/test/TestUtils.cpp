...
std::vector<float> randVecs(size_t num, size_t dim) {
    ...
    faiss_navix::float_rand(v.data(), v.size(), s_seed);
...
std::vector<unsigned char> randBinaryVecs(size_t num, size_t dim) {
    ...
    faiss_navix::byte_rand(v.data(), v.size(), s_seed);
    // unfortunately we generate separate sets of vectors, and don't
    // want the same values
    ++s_seed;
...
void compareIndices(
        ...
        faiss_navix::Index& refIndex,
        faiss_navix::Index& testIndex,
        int numQuery,
        int /*dim*/,
        int k,
    ...
    std::vector<faiss_navix::idx_t> refIndices(numQuery * k, -1);
    refIndex.search(
            numQuery,
            queryVecs.data(),
            k,
    ...
    std::vector<faiss_navix::idx_t> testIndices(numQuery * k, -1);
    testIndex.search(
            numQuery,
            queryVecs.data(),
            k,
...
void compareIndices(
        ...
        float pctMaxDiff1,
        float pctMaxDiffN) {
    auto queryVecs = faiss_navix::gpu::randVecs(numQuery, dim);
...
void compareLists(
        const float* refDist,
        const faiss_navix::idx_t* refInd,
        const float* testDist,
        const faiss_navix::idx_t* testInd,
        int dim1,
        int dim2,
        const std::string& configMsg,
    ...
    std::vector<std::unordered_map<faiss_navix::idx_t, int>> refIndexMap;

    for (int query = 0; query < dim1; ++query) {
        std::unordered_map<faiss_navix::idx_t, int> indices;
    ...
    for (int query = 0; query < dim1; ++query) {
        ...
        std::set<faiss_navix::idx_t> uniqueIndices;
