...
#include <faiss_navix/gpu/impl/InterleavedCodes.h>
#include <faiss_navix/gpu/utils/DeviceUtils.h>
#include <faiss_navix/gpu/utils/StaticUtils.h>
...
std::vector<uint8_t> unpackNonInterleaved(
    ...
    FAISS_NAVIX_ASSERT(data.size() == numVecs * srcVecSize);
...
#pragma omp parallel for
        for (int i = 0; i < numVecs; ++i) {
            for (int j = 0; j < dims; ++j) {
                ...
                FAISS_NAVIX_ASSERT(srcIdx < data.size());
...
#pragma omp parallel for
        for (int i = 0; i < numVecs; ++i) {
            for (int j = 0; j < dims; ++j) {
                ...
                FAISS_NAVIX_ASSERT(lo < data.size());
                FAISS_NAVIX_ASSERT(hi <= data.size());
...
#pragma omp parallel for
    for (int i = 0; i < numVecs; ++i) {
        ...
        FAISS_NAVIX_ASSERT(block < numBlocks);
        int lane = i % warpSize;
...
std::vector<uint8_t> unpackInterleaved(
    ...
    FAISS_NAVIX_ASSERT(data.size() == totalSize);
...
#pragma omp parallel for
        for (int i = 0; i < numVecs; ++i) {
            ...
            for (int j = 0; j < dims; ++j) {
                ...
                FAISS_NAVIX_ASSERT(lo < bytesPerDimBlock);
                FAISS_NAVIX_ASSERT(hi <= bytesPerDimBlock);
...
inline uint8_t pack5(int i, uint8_t lo, uint8_t hi, uint8_t hi2) {
    FAISS_NAVIX_ASSERT((lo & 0x1f) == lo);
    FAISS_NAVIX_ASSERT((hi & 0x1f) == hi);
    FAISS_NAVIX_ASSERT((hi2 & 0x1f) == hi2);
...
inline uint8_t pack6(int i, uint8_t lo, uint8_t hi) {
    FAISS_NAVIX_ASSERT((lo & 0x3f) == lo);
    FAISS_NAVIX_ASSERT((hi & 0x3f) == hi);
...
std::vector<uint8_t> packNonInterleaved(
        ...
        int bitsPerCode) {
    // bit codes padded to whole bytes
    FAISS_NAVIX_ASSERT(data.size() == numVecs * dims * utils::divUp(bitsPerCode, 8));
...
#pragma omp parallel for
        for (int i = 0; i < numVecs; ++i) {
            for (int j = 0; j < bytesPerVec; ++j) {
                int dimLo = j * 2;
                int dimHi = dimLo + 1;
                FAISS_NAVIX_ASSERT(dimLo < dims);
                FAISS_NAVIX_ASSERT(dimHi <= dims);
...
#pragma omp parallel for
        for (int i = 0; i < numVecs; ++i) {
            for (int j = 0; j < bytesPerVec; ++j) {
                ...
                FAISS_NAVIX_ASSERT(dimLo < dims);
                FAISS_NAVIX_ASSERT(dimHi <= dims);
                FAISS_NAVIX_ASSERT(dimHi <= dims + 1);
