...
#include <faiss_navix/gpu/impl/DistanceUtils.cuh>
#include <faiss_navix/gpu/utils/BlockSelectKernel.cuh>
#include <faiss_navix/gpu/utils/ConversionOperators.cuh>
#include <faiss_navix/gpu/utils/DeviceDefs.cuh>
#include <faiss_navix/gpu/utils/DeviceTensor.cuh>
#include <faiss_navix/gpu/utils/Select.cuh>
...
void runGeneralDistanceKernel(
        ...
        const DistanceOp& op,
        cudaStream_t stream) {
    FAISS_NAVIX_ASSERT(vecs.getSize(1) == query.getSize(1));
    FAISS_NAVIX_ASSERT(out.getSize(0) == query.getSize(0));
    FAISS_NAVIX_ASSERT(out.getSize(1) == vecs.getSize(0));

    dim3 grid(
            utils::divUp(vecs.getSize(0), TILE_SIZE),
            utils::divUp(query.getSize(0), TILE_SIZE));
    FAISS_NAVIX_ASSERT(grid.y <= getMaxGridCurrentDevice().y);
...
void runGeneralDistance(
    ...
    FAISS_NAVIX_ASSERT(outDistances.getSize(0) == numQueries);
    FAISS_NAVIX_ASSERT(outIndices.getSize(0) == numQueries);
    FAISS_NAVIX_ASSERT(outDistances.getSize(1) == k);
    FAISS_NAVIX_ASSERT(outIndices.getSize(1) == k);
    ...
    FAISS_NAVIX_ASSERT(k <= GPU_MAX_SELECTION_K); // select limitation
    ...
    if (interrupt) {
        FAISS_NAVIX_THROW_MSG("interrupted");
    }
