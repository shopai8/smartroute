...
#include <faiss_navix/gpu/impl/RemapIndices.h>
...
void ivfOffsetToUserIndex(
    ...
    FAISS_NAVIX_ASSERT(numLists == listOffsetToUserIndex.size());
...
#pragma omp parallel for
    for (idx_t q = 0; q < queries; ++q) {
        for (idx_t r = 0; r < k; ++r) {
            ...
            FAISS_NAVIX_ASSERT(listId < numLists);
            auto& listIndices = listOffsetToUserIndex[listId];

            FAISS_NAVIX_ASSERT(listOffset < listIndices.size());
