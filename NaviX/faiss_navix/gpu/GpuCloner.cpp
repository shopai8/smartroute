...
#include <faiss_navix/IndexReplicas.h>
#include <faiss_navix/IndexScalarQuantizer.h>
#include <faiss_navix/IndexShardsIVF.h>
#include <faiss_navix/MetaIndexes.h>
#include <faiss_navix/gpu/GpuIndex.h>
#include <faiss_navix/gpu/GpuIndexBinaryFlat.h>
#if defined USE_NVIDIA_CUVS
#include <faiss_navix/gpu/GpuIndexCagra.h>
#endif
...
void ToCPUCloner::merge_index(Index* dst, Index* src, bool successive_ids) {
    if (auto ifl = dynamic_cast<IndexFlat*>(dst)) {
        ...
        FAISS_NAVIX_ASSERT(ifl2);
        FAISS_NAVIX_ASSERT(successive_ids);
        ...
        FAISS_NAVIX_ASSERT(!"merging not implemented for this type of class");
    }
}
...
Index* ToCPUCloner::clone_Index(const Index* index) {
    if (auto ifl = dynamic_cast<const GpuIndexFlat*>(index)) {
        ...
        FAISS_NAVIX_ASSERT(ipr->count() > 0);
...
Index* ToGpuCloner::clone_Index(const Index* index) {
    if (auto ifl = dynamic_cast<const IndexFlat*>(index)) {
        ...
        return gif;
    } else if (auto ifl = dynamic_cast<const faiss_navix::IndexIVFFlat*>(index)) {
        ...
        return res;
            ...
            auto ifl = dynamic_cast<const faiss_navix::IndexIVFScalarQuantizer*>(
                    index)) {
        ...
        res->copyFrom(ifl);
        return res;
    } else if (auto ipq = dynamic_cast<const faiss_navix::IndexIVFPQ*>(index)) {
        if (verbose) {
...
#if defined USE_NVIDIA_CUVS
    else if (auto icg = dynamic_cast<const faiss_navix::IndexHNSWCagra*>(index)) {
        ...
        FAISS_NAVIX_THROW_MSG("This index type is not implemented on GPU.");
    }
}
...
ToGpuClonerMultiple::ToGpuClonerMultiple(
    ...
    FAISS_NAVIX_THROW_IF_NOT(provider.size() == devices.size());
...
void ToGpuClonerMultiple::copy_ivf_shard(
    ...
    if (shard_type == 2) {
        ...
        FAISS_NAVIX_ASSERT(idx2->ntotal == i1 - i0);
        ...
        FAISS_NAVIX_THROW_FMT("shard_type %d not implemented", shard_type);
    }
}
...
Index* ToGpuClonerMultiple::clone_Index_to_shards(const Index* index) {
    ...
    auto index_ivf = dynamic_cast<const faiss_navix::IndexIVF*>(index);
    auto index_ivfpq = dynamic_cast<const faiss_navix::IndexIVFPQ*>(index);
    auto index_ivfflat = dynamic_cast<const faiss_navix::IndexIVFFlat*>(index);
    auto index_ivfsq =
            dynamic_cast<const faiss_navix::IndexIVFScalarQuantizer*>(index);
    auto index_flat = dynamic_cast<const faiss_navix::IndexFlat*>(index);
    FAISS_NAVIX_THROW_IF_NOT_MSG(
    ...
    std::vector<faiss_navix::Index*> shards(n);
...
#pragma omp parallel for
    for (idx_t i = 0; i < n; i++) {
        ...
        if (index_ivfpq) {
            ...
            faiss_navix::IndexIVFScalarQuantizer idx2(
                    const_cast<Index*>(quantizer),
                    index->d,
                    index_ivfsq->nlist,
            ...
            faiss_navix::IndexFlat idx2(index->d, index->metric_type);
            shards[i] = sub_cloners[i].clone_Index(&idx2);
            if (index->ntotal > 0) {
    ...
    FAISS_NAVIX_ASSERT(index->ntotal == res->ntotal);
    return res;
}
...
GpuProgressiveDimIndexFactory::GpuProgressiveDimIndexFactory(int ngpu) {
    FAISS_NAVIX_THROW_IF_NOT(ngpu >= 1);
    devices.resize(ngpu);
    vres.resize(ngpu);
...
faiss_navix::IndexBinary* index_binary_gpu_to_cpu(
    ...
    if (auto ii = dynamic_cast<const GpuIndexBinaryFlat*>(gpu_index)) {
        ...
        FAISS_NAVIX_THROW_MSG("cannot clone this type of index");
    }
}
...
faiss_navix::IndexBinary* index_binary_cpu_to_gpu_multiple(
        ...
        const GpuMultipleClonerOptions* options) {
    GpuMultipleClonerOptions defaults;
    FAISS_NAVIX_THROW_IF_NOT(devices.size() == provider.size());
    int n = devices.size();
    if (n == 1) {
    ...
    if (options->shard) {
        ...
        FAISS_NAVIX_THROW_IF_NOT_MSG(fi, "only flat index cloning supported");
