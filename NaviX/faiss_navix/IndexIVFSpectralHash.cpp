...
void IndexIVFSpectralHash::train_encoder(
    ...
    for (size_t i = 0; i < n; i++) {
        FAISS_NAVIX_THROW_IF_NOT(idx[i] >= 0);
        sizes[idx[i]]++;
    }
...
struct IVFScanner : InvertedListScanner {
    ...
    void set_query(const float* query) override {
        FAISS_NAVIX_THROW_IF_NOT(query);
        FAISS_NAVIX_THROW_IF_NOT(q.size() == nbit);
...
void IndexIVFSpectralHash::replace_vt(VectorTransform* vt_in, bool own) {
    FAISS_NAVIX_THROW_IF_NOT(vt_in->d_out == nbit);
    FAISS_NAVIX_THROW_IF_NOT(vt_in->d_in == d);
    if (own_fields) {
        delete vt;
    }
    vt = vt_in;
...
void IndexIVFSpectralHash::replace_vt(IndexPreTransform* encoder, bool own) {
    FAISS_NAVIX_THROW_IF_NOT(encoder->chain.size() == 1);
    auto sub_index = dynamic_cast<IndexLSH*>(encoder->index);
    FAISS_NAVIX_THROW_IF_NOT_MSG(sub_index, "final index should be LSH");
    FAISS_NAVIX_THROW_IF_NOT(sub_index->nbits == nbit);
    FAISS_NAVIX_THROW_IF_NOT(!sub_index->rotate_data);
    FAISS_NAVIX_THROW_IF_NOT(!sub_index->train_thresholds);
