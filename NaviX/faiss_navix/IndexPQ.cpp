...
#include <faiss_navix/impl/code_distance/code_distance.h>
...
void IndexPQ::search(
    ...
    if (iparams) {
        ...
        FAISS_NAVIX_THROW_IF_NOT_MSG(params, "invalid search params");
        FAISS_NAVIX_THROW_IF_NOT_MSG(!params->sel, "selector not supported");
    ...
    if (param_search_type == ST_PQ) { // Simple PQ search
        ...
        if (!encode_signs) {
            ...
            FAISS_NAVIX_THROW_IF_NOT(d == pq.nbits * pq.M);
...
void IndexPQ::search_core_polysemous(
    ...
    FAISS_NAVIX_THROW_IF_NOT(k > 0);
    FAISS_NAVIX_THROW_IF_NOT(pq.nbits == 8);
...
void IndexPQ::hamming_distance_histogram(
        ...
        int64_t* hist) {
    FAISS_NAVIX_THROW_IF_NOT(metric_type == METRIC_L2);
    FAISS_NAVIX_THROW_IF_NOT(pq.code_size % 8 == 0);
    FAISS_NAVIX_THROW_IF_NOT(pq.nbits == 8);
