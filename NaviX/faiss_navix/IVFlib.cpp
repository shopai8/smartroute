...
#include <faiss_navix/IndexAdditiveQuantizer.h>
#include <faiss_navix/IndexIVFAdditiveQuantizer.h>
#include <faiss_navix/IndexIVFIndependentQuantizer.h>
#include <faiss_navix/IndexPreTransform.h>
#include <faiss_navix/IndexRefine.h>
#include <faiss_navix/MetaIndexes.h>
...
void check_compatible_for_merge(const Index* index0, const Index* index1) {
    const faiss_navix::IndexPreTransform* pt0 =
            dynamic_cast<const faiss_navix::IndexPreTransform*>(index0);

    if (pt0) {
        const faiss_navix::IndexPreTransform* pt1 =
                dynamic_cast<const faiss_navix::IndexPreTransform*>(index1);
        FAISS_NAVIX_THROW_IF_NOT_MSG(pt1, "both indexes should be pretransforms");

        FAISS_NAVIX_THROW_IF_NOT(pt0->chain.size() == pt1->chain.size());
        for (int i = 0; i < pt0->chain.size(); i++) {
            FAISS_NAVIX_THROW_IF_NOT(typeid(pt0->chain[i]) == typeid(pt1->chain[i]));
        }

        index0 = pt0->index;
        index1 = pt1->index;
    }
    FAISS_NAVIX_THROW_IF_NOT(typeid(index0) == typeid(index1));
    FAISS_NAVIX_THROW_IF_NOT(
            index0->d == index1->d &&
            index0->metric_type == index1->metric_type);

    const faiss_navix::IndexIVF* ivf0 = dynamic_cast<const faiss_navix::IndexIVF*>(index0);
    if (ivf0) {
        const faiss_navix::IndexIVF* ivf1 =
                dynamic_cast<const faiss_navix::IndexIVF*>(index1);
        FAISS_NAVIX_THROW_IF_NOT(ivf1);
...
void merge_into(faiss_navix::Index* index0, faiss_navix::Index* index1, bool shift_ids) {
...
void search_centroid(
    ...
    if (auto index_pre = dynamic_cast<faiss_navix::IndexPreTransform*>(index)) {
        x = index_pre->apply_chain(n, x);
        del.reset((float*)x);
        index = index_pre->index;
    }
    faiss_navix::IndexIVF* index_ivf = dynamic_cast<faiss_navix::IndexIVF*>(index);
    assert(index_ivf);
...
void SlidingIndexWindow::step(const Index* sub_index, bool remove_oldest) {
    ...
    if (sub_index) {
        ...
        FAISS_NAVIX_THROW_IF_NOT_MSG(ils2, "supports only ArrayInvertedLists");
    }
    ...
    if (remove_oldest && ils2) {
        ...
        FAISS_NAVIX_THROW_MSG("nothing to do???");
    }
...
ArrayInvertedLists* get_invlist_range(const Index* index, long i0, long i1) {
    ...
    FAISS_NAVIX_THROW_IF_NOT(0 <= i0 && i0 <= i1 && i1 <= ivf->nlist);
...
void set_invlist_range(
    ...
    FAISS_NAVIX_THROW_IF_NOT_MSG(dst, "only ArrayInvertedLists supported");
    FAISS_NAVIX_THROW_IF_NOT(
...
void search_with_parameters(
        ...
        size_t* nb_dis_ptr,
        double* ms_per_stage) {
    FAISS_NAVIX_THROW_IF_NOT(params);
    ...
    FAISS_NAVIX_THROW_IF_NOT(index_ivf);
...
IndexIVFResidualQuantizer* ivf_residual_from_quantizer(
        ...
        int nlevel) {
    FAISS_NAVIX_THROW_IF_NOT(nlevel > 0 && nlevel + 1 < rq.M);
...
void ivf_residual_add_from_flat_codes(
    ...
    const ResidualCoarseQuantizer* rcq =
            dynamic_cast<const faiss_navix::ResidualCoarseQuantizer*>(
                    index->quantizer);
    FAISS_NAVIX_THROW_IF_NOT_MSG(rcq, "the coarse quantizer must be a RCQ");
    if (code_size < 0) {
...
void handle_ivf(
        faiss_navix::IndexIVF* index,
        int64_t shard_count,
        const std::string& filename_template,
        ShardingFunction* sharding_function,
        bool generate_ids) {
    std::vector<faiss_navix::IndexIVF*> sharded_indexes(shard_count);
    auto clone = static_cast<faiss_navix::IndexIVF*>(faiss_navix::clone_index(index));
    clone->quantizer->reset();
    for (int64_t i = 0; i < shard_count; i++) {
        sharded_indexes[i] =
                static_cast<faiss_navix::IndexIVF*>(faiss_navix::clone_index(clone));
        if (generate_ids) {
    ...
    for (int64_t i = 0; i < shard_count; i++) {
        ...
        faiss_navix::write_index(sharded_indexes[i], fname);
    }
...
void handle_binary_ivf(
        faiss_navix::IndexBinaryIVF* index,
        int64_t shard_count,
        const std::string& filename_template,
        ShardingFunction* sharding_function,
        bool generate_ids) {
    std::vector<faiss_navix::IndexBinaryIVF*> sharded_indexes(shard_count);

    auto clone = static_cast<faiss_navix::IndexBinaryIVF*>(
            faiss_navix::clone_binary_index(index));
    clone->quantizer->reset();

    for (int64_t i = 0; i < shard_count; i++) {
        sharded_indexes[i] = static_cast<faiss_navix::IndexBinaryIVF*>(
                faiss_navix::clone_binary_index(clone));
        if (generate_ids) {
            // Assume the quantizer does not natively support add_with_ids.
            sharded_indexes[i]->quantizer =
    ...
    for (int64_t i = 0; i < shard_count; i++) {
        char fname[256];
        snprintf(fname, 256, filename_template.c_str(), i);
        faiss_navix::write_index_binary(sharded_indexes[i], fname);
    }
...
void sharding_helper(
        ...
        ShardingFunction* sharding_function,
        bool generate_ids) {
    FAISS_NAVIX_THROW_IF_MSG(index->quantizer->ntotal == 0, "No centroids to shard.");
    FAISS_NAVIX_THROW_IF_MSG(
    ...
    if (typeid(IndexType) == typeid(faiss_navix::IndexIVF)) {
        handle_ivf(
                dynamic_cast<faiss_navix::IndexIVF*>(index),
                shard_count,
                filename_template,
                sharding_function,
                generate_ids);
    } else if (typeid(IndexType) == typeid(faiss_navix::IndexBinaryIVF)) {
        handle_binary_ivf(
                dynamic_cast<faiss_navix::IndexBinaryIVF*>(index),
                shard_count,
                filename_template,
                sharding_function,
                generate_ids);
