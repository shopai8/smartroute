...
#include <faiss_navix/python/python_callbacks.h>
...
size_t PyCallbackIOReader::operator()(void* ptrv, size_t size, size_t nitems) {
    ...
    while (rs > 0) {
        ...
        if (result == nullptr) {
            FAISS_NAVIX_THROW_MSG("propagate py error");
        }
        if (!PyBytes_Check(result)) {
            Py_DECREF(result);
            FAISS_NAVIX_THROW_MSG("read callback did not return a bytes object");
        }
...
bool PyCallbackIDSelector::is_member(faiss_navix::idx_t id) const {
    FAISS_NAVIX_THROW_IF_NOT((id >> 32) == 0);
    PyThreadLock gil;
