...
#include <faiss_navix/IndexIVFFastScan.h>
...
#include <faiss_navix/impl/LookupTableScaler.h>
#include <faiss_navix/impl/pq4_fast_scan.h>
#include <faiss_navix/impl/simd_result_handlers.h>
#include <faiss_navix/invlists/BlockInvertedLists.h>
#include <faiss_navix/utils/hamming.h>
#include <faiss_navix/utils/quantize_lut.h>
...
void IndexIVFFastScan::init_fastscan(
        ...
        MetricType /* metric */,
        int bbs_2) {
    FAISS_NAVIX_THROW_IF_NOT(bbs_2 % 32 == 0);
    FAISS_NAVIX_THROW_IF_NOT(nbits_init == 4);
    FAISS_NAVIX_THROW_IF_NOT(fine_quantizer->d == d);
    ...
    M2 = roundup(M, 2);
    code_size = M2 / 2;
    FAISS_NAVIX_THROW_IF_NOT(code_size == fine_quantizer->code_size);
...
void IndexIVFFastScan::add_with_ids(
    ...
    FAISS_NAVIX_THROW_IF_NOT_MSG(bil, "only block inverted lists supported");
...
void IndexIVFFastScan::search_preassigned(
    ...
    if (params) {
        FAISS_NAVIX_THROW_IF_NOT(params->max_codes == 0);
        nprobe = params->nprobe;
    }

    FAISS_NAVIX_THROW_IF_NOT_MSG(
            !store_pairs, "store_pairs not supported for this index");
    FAISS_NAVIX_THROW_IF_NOT_MSG(!stats, "stats not supported for this index");
...
#pragma omp parallel for reduction(+ : ndis, nlist_visited)
        ...
        FAISS_NAVIX_THROW_FMT("implem %d does not exist", implem);
    }
}
...
void IndexIVFFastScan::range_search_dispatch_implem(
    ...
    if (!multiple_threads) { // single thread
        ...
        if (impl == 12) {
            ...
            FAISS_NAVIX_THROW_FMT("Range search implem %d not implemented", impl);
        }
    } else {
        // explicitly slice over threads
        int nslice = compute_search_nslice(this, n, cq.nprobe);
...
void IndexIVFFastScan::search_implem_1(
        ...
        const NormTableScaler* scaler,
        const IVFSearchParameters* params) const {
    FAISS_NAVIX_THROW_IF_NOT(orig_invlists);
...
void IndexIVFFastScan::search_implem_12(
    ...
    FAISS_NAVIX_THROW_IF_NOT(bbs == 32);
...
void IndexIVFFastScan::reconstruct_orig_invlists() {
    FAISS_NAVIX_THROW_IF_NOT(orig_invlists != nullptr);
    FAISS_NAVIX_THROW_IF_NOT(orig_invlists->list_size(0) == 0);
