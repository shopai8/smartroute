...
void sync_d(IndexBinary* index) {
    FAISS_NAVIX_THROW_IF_NOT(index->d % 8 == 0);
...
void IndexReplicasTemplate<IndexT>::reconstruct(idx_t n, component_t* x) const {
    FAISS_NAVIX_THROW_IF_NOT_MSG(this->count() > 0, "no replicas in index");
...
void IndexReplicasTemplate<IndexT>::search(
    ...
    faiss_navix::idx_t queriesPerIndex =
            (faiss_navix::idx_t)(n + this->count() - 1) / (faiss_navix::idx_t)this->count();
    FAISS_NAVIX_ASSERT(n / queriesPerIndex <= this->count());

    auto fn = [queriesPerIndex, componentsPerVec, n, x, k, distances, labels](
                      int i, const IndexT* index) {
        faiss_navix::idx_t base = (faiss_navix::idx_t)i * queriesPerIndex;
...
void IndexReplicasTemplate<IndexT>::syncWithSubIndexes() {
    ...
    for (int i = 1; i < this->count(); ++i) {
        ...
        FAISS_NAVIX_THROW_IF_NOT(this->metric_type == index->metric_type);
        FAISS_NAVIX_THROW_IF_NOT(this->d == index->d);
        FAISS_NAVIX_THROW_IF_NOT(this->is_trained == index->is_trained);
        FAISS_NAVIX_THROW_IF_NOT(this->ntotal == index->ntotal);
    }
}
