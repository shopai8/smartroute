...
#include <faiss_navix/utils/extra_distances.h>
...
IndexAdditiveQuantizer::IndexAdditiveQuantizer(
    ...
    FAISS_NAVIX_THROW_IF_NOT(metric == METRIC_INNER_PRODUCT || metric == METRIC_L2);
}
...
FlatCodesDistanceComputer* IndexAdditiveQuantizer::
    ...
    if (aq->search_type == AdditiveQuantizer::ST_decompress) {
        if (metric_type == METRIC_L2) {
            ...
            FAISS_NAVIX_THROW_MSG("unsupported metric");
        }
    } else {
...
void AdditiveCoarseQuantizer::add(idx_t, const float*) {
    FAISS_NAVIX_THROW_MSG("not applicable");
}
...
void AdditiveCoarseQuantizer::search(
    ...
    if (metric_type == METRIC_INNER_PRODUCT) {
        ...
        FAISS_NAVIX_THROW_IF_NOT(centroid_norms.size() == ntotal);
...
ResidualCoarseQuantizer::ResidualCoarseQuantizer(
    ...
    FAISS_NAVIX_THROW_IF_NOT(rq.tot_bits <= 63);
    is_trained = false;
}
...
void ResidualCoarseQuantizer::set_beam_factor(float new_beam_factor) {
    ...
    if (new_beam_factor > 0) {
        FAISS_NAVIX_THROW_IF_NOT(new_beam_factor >= 1.0);
...
void ResidualCoarseQuantizer::initialize_from(
    ...
    FAISS_NAVIX_THROW_IF_NOT(rq.M <= other.rq.M);
...
LocalSearchCoarseQuantizer::LocalSearchCoarseQuantizer(
    ...
    FAISS_NAVIX_THROW_IF_NOT(lsq.tot_bits <= 63);
    is_trained = false;
}
