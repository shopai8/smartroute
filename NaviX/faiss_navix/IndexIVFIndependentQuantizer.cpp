...
IndexIVFIndependentQuantizer::IndexIVFIndependentQuantizer(
    ...
    if (vt) {
        ...
        FAISS_NAVIX_THROW_IF_NOT_MSG(index_ivf->d == d, "invalid vector dimensions");
    }

    if (quantizer->is_trained && quantizer->ntotal != 0) {
        FAISS_NAVIX_THROW_IF_NOT(quantizer->ntotal == index_ivf->nlist);
    }
    if (index_ivf->is_trained && vt) {
        FAISS_NAVIX_THROW_IF_NOT(vt->is_trained);
    }
...
void IndexIVFIndependentQuantizer::search(
    ...
    FAISS_NAVIX_THROW_IF_NOT_MSG(!params, "search parameters not supported");
