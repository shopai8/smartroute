...
void Index2Layer::transfer_to_IVFPQ(IndexIVFPQ& other) const {
    FAISS_NAVIX_THROW_IF_NOT(other.nlist == q1.nlist);
    FAISS_NAVIX_THROW_IF_NOT(other.code_size == code_size_2);
    FAISS_NAVIX_THROW_IF_NOT(other.ntotal == 0);
...
struct Distance2Level : DistanceComputer {
    ...
    explicit Distance2Level(const Index2Layer& storage) : storage(storage) {
        d = storage.d;
        FAISS_NAVIX_ASSERT(storage.pq.dsub == 4);
...
#else
        FAISS_NAVIX_THROW_MSG("not implemented for non-x64 platforms");
#endif
    }
};
...
struct Distance2xXPQ4 : Distance2Level {
    ...
    explicit Distance2xXPQ4(const Index2Layer& storage)
        ...
        FAISS_NAVIX_ASSERT(mi);
        FAISS_NAVIX_ASSERT(storage.pq.M % 2 == 0);
        M_2 = storage.pq.M / 2;
