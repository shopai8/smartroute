...
std::vector<float> get_random_vectors(size_t n, int seed) {
    ...
    faiss_navix::rand_smooth_vectors(n, d, x.data(), seed);
    seed++;
    return x;
}
...
struct DispatchingInvertedLists : faiss_navix::ReadOnlyInvertedLists {
    DispatchingInvertedLists(size_t nlist, size_t code_size)
            : faiss_navix::ReadOnlyInvertedLists(nlist, code_size) {
        use_iterator = true;
    }

    faiss_navix::InvertedListsIterator* get_iterator(
            size_t list_no,
            void* inverted_list_context = nullptr) const override {
        assert(inverted_list_context);
        auto il =
                static_cast<const faiss_navix::InvertedLists*>(inverted_list_context);
    ...
    size_t list_size(size_t list_no) const override {
        FAISS_NAVIX_THROW_MSG("use iterator interface");
    }
...
TEST(COMMON, test_common_trained_index) {
    ...
    std::unique_ptr<faiss_navix::IndexIVF> empty_index(dynamic_cast<faiss_navix::IndexIVF*>(
            faiss_navix::index_factory(d, "IVF32,PQ8np")));
    ...
    // results
    std::vector<std::vector<faiss_navix::idx_t>> ref_I(N);

    for (int i = 0; i < N; i++) {
        // clone the empty index
        std::unique_ptr<faiss_navix::Index> index(
                faiss_navix::clone_index(empty_index.get()));
    ...
    std::vector<faiss_navix::ArrayInvertedLists> sub_invlists;

    for (int i = 0; i < N; i++) {
        // swap in other inverted lists
        sub_invlists.emplace_back(empty_index->nlist, empty_index->code_size);
        faiss_navix::InvertedLists* invlists = &sub_invlists.back();
    ...
    std::vector<std::vector<faiss_navix::idx_t>> new_I(N);
