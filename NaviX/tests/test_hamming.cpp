...
TEST(TestHamming, test_crosshamming_count_thres) {
    ...
    for (auto ncodes : {8, 16, 32, 64, 65}) {
        ...
        for (int i = 0; i < n; ++i) {
            uint64_t* bs2 = bs1 + 2;
            for (int j = i + 1; j < n; ++j) {
                if (faiss_navix::hamming(bs1 + i * nwords, bs2 + j * nwords, nwords) <
                    hamming_threshold) {
                    ++true_count;
                }
            }
        ...
        if (ncodes == 65) {
            ASSERT_THROW(
                    faiss_navix::crosshamming_count_thres(
                            dbs.data(), n, hamming_threshold, ncodes, &count),
                    faiss_navix::FaissException);
            continue;
        }
        faiss_navix::crosshamming_count_thres(
                dbs.data(), n, hamming_threshold, ncodes, &count);
...
TEST(TestHamming, test_hamming_thres) {
    ...
    for (auto ncodes : {8, 16, 32, 64, 65}) {
        ...
        for (int i = 0; i < n1; ++i) {
            for (int j = 0; j < n2; ++j) {
                hamdis_t ham_dist = faiss_navix::hamming(
        ...
        if (ncodes == 65) {
            ASSERT_THROW(
                    faiss_navix::match_hamming_thres(
                            bs1.data(),
                            bs2.data(),
                            n1,
                            n2,
                    ...
                    faiss_navix::FaissException);
            ASSERT_THROW(
                    faiss_navix::hamming_count_thres(
                            bs1.data(),
                            bs2.data(),
                            n1,
                            n2,
        ...
        size_t match_count = faiss_navix::match_hamming_thres(
                bs1.data(),
                bs2.data(),
                n1,
                n2,
...
TEST(TestHamming, test_hamming_knn) {
    ...
    for (auto code_size : {8, 16, 24, 32}) {
        ...
        faiss_navix::int_maxheap_array_t res = {
                na, k, ids_gen.data(), dist_gen.data()};
        faiss_navix::generalized_hammings_knn_hc(
        ...
        faiss_navix::hammings_knn(&res, a->data(), b->data(), nb, code_size, true);
    ...
    for (auto code_size : {8, 16, 24, 32}) {
        std::stringstream assert_str = get_correct_hamming_example(
                na,
                nb,
                /* k */ nb, // faiss_navix::hammings computes all distances
                code_size,
                a,
                b,
                true_ids,
