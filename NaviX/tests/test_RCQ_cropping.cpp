...
TEST(RCQCropping, test_cropping) {
    ...
    faiss_navix::rand_smooth_vectors(nq + nb + nt, d, buf.data(), 1234);
    const float* xt = buf.data();
    const float* xb = xt + nt * d;
    const float* xq = xb + nb * d;

    std::vector<size_t> nbits = {5, 4, 4};
    faiss_navix::ResidualCoarseQuantizer rcq(d, nbits);
    ...
    // perform search
    int nprobe = 15;
    std::vector<faiss_navix::idx_t> Iref(nq * nprobe);
    ...
    nbits.pop_back();
    faiss_navix::ResidualCoarseQuantizer rcq_cropped(d, nbits);
    rcq_cropped.initialize_from(rcq);

    EXPECT_EQ(rcq_cropped.ntotal, rcq.ntotal >> last_nbits);

    // perform search
    std::vector<faiss_navix::idx_t> Inew(nq * nprobe);
...
TEST(RCQCropping, search_params) {
    ...
    faiss_navix::ResidualCoarseQuantizer quantizer(d, nbits);
    size_t ntotal = (size_t)1 << quantizer.rq.tot_bits;
    faiss_navix::IndexIVFScalarQuantizer index(
            &quantizer, d, ntotal, faiss_navix::ScalarQuantizer::QT_8bit);
    ...
    faiss_navix::SearchParametersResidualCoarseQuantizer params1;
