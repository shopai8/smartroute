...
#include <faiss_navix/Index2Layer.h>
#include <faiss_navix/IndexIVFPQ.h>
#include <faiss_navix/IndexPQ.h>
#include <faiss_navix/impl/io.h>
#include <faiss_navix/index_factory.h>
#include <faiss_navix/index_io.h>

#include <faiss_navix/IndexRowwiseMinMax.h>
#include <faiss_navix/cppcontrib/SaDecodeKernels.h>

using namespace ::testing;
using ::testing::TestWithParam;
using ::testing::Values;

std::tuple<std::shared_ptr<faiss_navix::Index>, std::vector<uint8_t>> trainDataset(
        const std::vector<float>& input,
        const uint64_t n,
        const uint64_t d,
        const std::string& description) {
    // train an index
    auto index = std::shared_ptr<faiss_navix::Index>(
            faiss_navix::index_factory((int)d, description.c_str()));
...
bool testIfIVFPQ(
        const faiss_navix::Index* const index,
    ...
    const faiss_navix::IndexIVFPQ* const indexQ =
            dynamic_cast<const faiss_navix::IndexIVFPQ*>(index);
    if (indexQ == nullptr) {
        return false;
    }

    const auto coarseIndexQ =
            dynamic_cast<const faiss_navix::IndexFlatCodes*>(indexQ->quantizer);
...
bool testIfResidualPQ(
    ...
    const faiss_navix::Index2Layer* const indexQ =
            dynamic_cast<const faiss_navix::Index2Layer*>(index);
    if (indexQ == nullptr) {
        return false;
    }

    const auto coarseIndexQ = dynamic_cast<const faiss_navix::MultiIndexQuantizer*>(
            indexQ->q1.quantizer);
    if (coarseIndexQ == nullptr) {
        return false;
    }
...
void verifyIndex2LevelDecoder(
        const uint64_t n,
        const uint64_t d,
        const std::shared_ptr<faiss_navix::Index>& index,
...
void verifyMinMaxIndex2LevelDecoder(
    ...
    const std::shared_ptr<faiss_navix::IndexRowwiseMinMaxBase> indexMinMax =
            std::dynamic_pointer_cast<faiss_navix::IndexRowwiseMinMaxBase>(index);
...
void verifyIndexPQDecoder(
        ...
        const std::vector<uint8_t>& encodedData) {
    //
    const faiss_navix::IndexPQ* const indexQ =
            dynamic_cast<const faiss_navix::IndexPQ*>(index.get());
...
void verifyMinMaxIndexPQDecoder(
    ...
    //
    const faiss_navix::IndexPQ* const indexQ =
            dynamic_cast<const faiss_navix::IndexPQ*>(subIndex);
    const float* const pqFineCentroidsQ = indexQ->pq.centroids.data();
...
void testIndex2LevelDecoder(
    ...
    std::shared_ptr<faiss_navix::Index> index;
...
TEST(testCppcontribSaDecode, D256_IVF1024_PQ16) {
    ...
    using T = faiss_navix::cppcontrib::Index2LevelDecoder<256, 256, 16, 16>;
...
TEST(testCppcontribSaDecode, D256_MINMAXFP16_IVF256_PQ16) {
    using SubT = faiss_navix::cppcontrib::Index2LevelDecoder<256, 256, 16>;
    using T = faiss_navix::cppcontrib::IndexMinMaxFP16Decoder<SubT>;
    testMinMaxIndex2LevelDecoder<T>(NSAMPLES, 256, "MinMaxFP16,IVF256,PQ16np");
}

TEST(testCppcontribSaDecode, D256_MINMAXFP16_PQ16) {
    using SubT = faiss_navix::cppcontrib::IndexPQDecoder<256, 16>;
    using T = faiss_navix::cppcontrib::IndexMinMaxFP16Decoder<SubT>;
...
TEST(testCppcontribSaDecode, D256_MINMAX_IVF256_PQ16) {
    using SubT = faiss_navix::cppcontrib::Index2LevelDecoder<256, 256, 16>;
    using T = faiss_navix::cppcontrib::IndexMinMaxDecoder<SubT>;
...
TEST(testCppcontribSaDecode, D256_MINMAXFP16_IVF1024_PQ16x10) {
    using SubT = faiss_navix::cppcontrib::Index2LevelDecoder<256, 256, 16, 10, 10>;
    using T = faiss_navix::cppcontrib::IndexMinMaxFP16Decoder<SubT>;
    testMinMaxIndex2LevelDecoder<T>(
...
TEST(testCppcontribSaDecode, D256_MINMAXFP16_IVF1024_PQ16x10_ALTERNATIVE) {
    using SubT = faiss_navix::cppcontrib::Index2LevelDecoder<256, 256, 16, 16, 10>;
    using T = faiss_navix::cppcontrib::IndexMinMaxFP16Decoder<SubT>;
    testMinMaxIndex2LevelDecoder<T>(
            NSAMPLES * 4, 256, "MinMaxFP16,IVF1024,PQ16x10np");
}
...
TEST(testCppcontribSaDecode, D160_Residual4x8_PQ8x10) {
    using T = faiss_navix::cppcontrib::Index2LevelDecoder<160, 40, 20, 8, 10>;
