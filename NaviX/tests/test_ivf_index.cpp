...
class TestContext {
   public:
    ...
    // id to codes map
    std::unordered_map<faiss_navix::idx_t, std::vector<uint8_t>> codes;
    // id to list_no map
    std::unordered_map<faiss_navix::idx_t, size_t> list_nos;
    faiss_navix::idx_t id = 0;
    std::set<size_t> lists_probed;
};

// the iterator that iterates over the codes stored in context object
class TestInvertedListIterator : public faiss_navix::InvertedListsIterator {
   public:
    ...
    virtual std::pair<faiss_navix::idx_t, const uint8_t*> get_id_and_codes()
            override {
        if (it == context->codes.cend()) {
            FAISS_NAVIX_THROW_MSG("invalid state");
        }
...
class TestInvertedLists : public faiss_navix::InvertedLists {
   public:
    TestInvertedLists(size_t nlist, size_t code_size)
            : faiss_navix::InvertedLists(nlist, code_size) {
        use_iterator = true;
    }

    ~TestInvertedLists() override {}
    size_t list_size(size_t /*list_no*/) const override {
        FAISS_NAVIX_THROW_MSG("unexpected call");
    }

    faiss_navix::InvertedListsIterator* get_iterator(size_t list_no, void* context)
            const override {
    ...
    const faiss_navix::idx_t* get_ids(size_t /* list_no */) const override {
        FAISS_NAVIX_THROW_MSG("unexpected call");
    }
...
TEST(IVF, list_context) {
    ...
    faiss_navix::IndexFlatL2 quantizer(d); // the other index
    faiss_navix::IndexIVFFlat index(&quantizer, d, nlist);
    ...
    constexpr faiss_navix::idx_t query_vector_id = 100;
    {
    ...
    {
        constexpr faiss_navix::idx_t k = 100;
        constexpr size_t nprobe = 10;
        std::vector<float> distances(k);
        std::vector<faiss_navix::idx_t> labels(k);
        faiss_navix::SearchParametersIVF params;
