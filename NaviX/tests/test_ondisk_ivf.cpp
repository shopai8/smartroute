...
TEST(ONDISK, make_invlists) {
    ...
    faiss_navix::OnDiskInvertedLists ivf(nlist, code_size, filename.c_str());
    ...
    for (int i = 0; i < nlist; i++) {
        ...
        const faiss_navix::idx_t* ids = ivf.get_ids(i);
        const uint8_t* codes = ivf.get_codes(i);
        for (int j = 0; j < size; j++) {
            faiss_navix::idx_t id = ids[j];
...
TEST(ONDISK, test_add) {
    ...
    faiss_navix::IndexFlatL2 quantizer(d);
    {
        ...
        faiss_navix::float_rand(x.data(), d * nlist, 12345);
        quantizer.add(nlist, x.data());
    }
    std::vector<float> xb(d * nb);
    faiss_navix::float_rand(xb.data(), d * nb, 23456);

    faiss_navix::IndexIVFFlat index(&quantizer, d, nlist);
    index.add(nb, xb.data());

    std::vector<float> xq(d * nb);
    faiss_navix::float_rand(xq.data(), d * nq, 34567);

    std::vector<float> ref_D(nq * k);
    std::vector<faiss_navix::idx_t> ref_I(nq * k);
    ...
    // test add + search
    {
        faiss_navix::IndexIVFFlat index2(&quantizer, d, nlist);

        faiss_navix::OnDiskInvertedLists ivf(
        ...
        std::vector<faiss_navix::idx_t> new_I(nq * k);
    ...
    // test io
    {
        faiss_navix::Index* index3 = faiss_navix::read_index(filename2.c_str());
