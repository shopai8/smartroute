...
#include <faiss_navix/cppcontrib/detail/UintReader.h>

template <intptr_t N_ELEMENTS, intptr_t CODE_BITS, intptr_t CPOS>
struct TestLoop {
    static void test(
            const uint8_t* const container,
            faiss_navix::BitstringReader& br) {
        // validate
        const intptr_t uintreader_data = faiss_navix::cppcontrib::detail::
...
void TestUintReader() {
    ...
    for (size_t attempt = 0; attempt < 10; attempt++) {
        ...
        // populate it
        faiss_navix::BitstringWriter bw(container.get(), CODE_BYTES);
        for (size_t i = 0; i < N_ELEMENTS; i++) {
            bw.write(u(rng), CODE_BITS);
        }

        // read it back and verify against bitreader
        faiss_navix::BitstringReader br(container.get(), CODE_BYTES);
