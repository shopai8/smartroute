...
std::unique_ptr<faiss_navix::Index> make_index(
        const char* index_type,
        const std::vector<float>& x) {
    auto index =
            std::unique_ptr<faiss_navix::Index>(faiss_navix::index_factory(d, index_type));
...
bool test_search_centroid(const char* index_key) {
    ...
    faiss_navix::ivflib::search_centroid(
            index.get(), xb.data(), nb, centroid_ids.data());

    const faiss_navix::IndexIVF* ivf = faiss_navix::ivflib::extract_index_ivf(index.get());
...
int test_search_and_return_centroids(const char* index_key) {
    ...
    faiss_navix::IndexIVF* ivf = faiss_navix::ivflib::extract_index_ivf(index.get());
    ivf->nprobe = 4;
    ...
    faiss_navix::ivflib::search_and_return_centroids(
            index.get(),
            nq,
            xq.data(),
            k,
