...
#include <faiss_navix/impl/ThreadedIndex.h>
...
struct MockIndex : public faiss_navix::Index {
    explicit MockIndex(idx_t d) : faiss_navix::Index(d) {
        resetMock();
    }
    ...
    void search(
            ...
            float* distances,
            idx_t* labels,
            const faiss_navix::SearchParameters* params) const override {
...
struct MockThreadedIndex : public faiss_navix::ThreadedIndex<IndexT> {
    using idx_t = faiss_navix::idx_t;

    explicit MockThreadedIndex(bool threaded)
            : faiss_navix::ThreadedIndex<IndexT>(threaded) {}
    ...
    void search(
            ...
            float*,
            idx_t*,
            const faiss_navix::SearchParameters*) const override {}
    void reset() override {}
};
...
TEST(ThreadedIndex, MultipleException) {
    ...
    for (bool threaded : {true, false}) {
        ...
        // FaissException
        EXPECT_THROW(ti.runOnIndex(fn), faiss_navix::FaissException);
...
TEST(ThreadedIndex, TestReplica) {
    ...
    for ([[maybe_unused]] const bool threaded : {true, false}) {
        std::vector<std::unique_ptr<MockIndex>> idxs;
        faiss_navix::IndexReplicas replica(d);
        ...
        std::vector<faiss_navix::idx_t> labels(n * k);
...
TEST(ThreadedIndex, TestShards) {
    ...
    for (bool threaded : {true, false}) {
        std::vector<std::unique_ptr<MockIndex>> idxs;
        faiss_navix::IndexShards shards(d, threaded);
