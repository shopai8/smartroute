...
TEST(PQEncoderGeneric, encode) {
    ...
    for (int nbits = minbits; nbits <= maxbits; ++nbits) {
        ...
        // mem.
        {
            faiss_navix::PQEncoderGeneric encoder(codes.get(), nbits);
            for (const auto& v : values) {
                encoder.encode(v & mask);
            }
        }

        faiss_navix::PQDecoderGeneric decoder(codes.get(), nbits);
...
TEST(PQEncoder8, encode) {
    ...
    faiss_navix::PQEncoder8 encoder(codes.get(), 8);
    for (const auto& v : values) {
        encoder.encode(v & mask);
    }

    faiss_navix::PQDecoder8 decoder(codes.get(), 8);
    for (int i = 0; i < nsubcodes; ++i) {
        uint64_t v = decoder.decode();
        EXPECT_EQ(values[i] & mask, v);
    }
...
TEST(PQEncoder16, encode) {
    ...
    faiss_navix::PQEncoder16 encoder(codes.get(), 16);
    for (const auto& v : values) {
        encoder.encode(v & mask);
    }

    faiss_navix::PQDecoder16 decoder(codes.get(), 16);
    for (int i = 0; i < nsubcodes; ++i) {
        uint64_t v = decoder.decode();
        EXPECT_EQ(values[i] & mask, v);
    }
...
TEST(PQFastScan, set_packed_element) {
    ...
    faiss_navix::IndexPQFastScan index(d, M, nbits);
    ...
    for (int j = 0; j < 10; j++) {
        ...
        for (int i = 0; i < ntotal; i++) {
            for (int sq = 0; sq < M; sq++) {
                old[i * M + sq] = faiss_navix::pq4_get_packed_element(
                        index.codes.data(), index.bbs, M, i, sq);
            }
        }
        for (int sq = 0; sq < M; sq++) {
            faiss_navix::pq4_set_packed_element(
                    index.codes.data(),
        ...
        for (int i = 0; i < ntotal; i++) {
            for (int sq = 0; sq < M; sq++) {
                uint8_t newcode = faiss_navix::pq4_get_packed_element(
                        index.codes.data(), index.bbs, M, i, sq);
