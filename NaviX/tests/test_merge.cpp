...
#include <faiss_navix/invlists/OnDiskInvertedLists.h>
...
typedef faiss_navix::idx_t idx_t;
...
struct CommonData {
    ...
    faiss_navix::IndexFlatL2 quantizer;
    ...
    CommonData() : database(nb * d), queries(nq * d), ids(nb), quantizer(d) {
        ...
        { // just to train the quantizer
            faiss_navix::IndexIVFFlat iflat(&quantizer, d, nlist);
...
/// compare results.
int compare_merged(
        faiss_navix::IndexShards* index_shards,
        bool shift_ids,
    ...
    if (standard_merge) {
        for (int i = 1; i < nindex; i++) {
            faiss_navix::ivflib::merge_into(
                    index_shards->at(0), index_shards->at(i), shift_ids);
        }

        index_shards->syncWithSubIndexes();
    } else {
        std::vector<const faiss_navix::InvertedLists*> lists;
        faiss_navix::IndexIVF* index0 = nullptr;
        size_t ntotal = 0;
        for (int i = 0; i < nindex; i++) {
            auto index_ivf =
                    dynamic_cast<faiss_navix::IndexIVF*>(index_shards->at(i));
            assert(index_ivf);
            if (i == 0) {
                index0 = index_ivf;
            }
        ...
        auto il = new faiss_navix::OnDiskInvertedLists(
...
TEST(MERGE, merge_flat_no_ids) {
    faiss_navix::IndexShards index_shards(d);
    index_shards.own_indices = true;
    for (int i = 0; i < nindex; i++) {
        index_shards.add_shard(
                new faiss_navix::IndexIVFFlat(&cd.quantizer, d, nlist));
    }
...
TEST(MERGE, merge_flat) {
    faiss_navix::IndexShards index_shards(d, false, false);
    index_shards.own_indices = true;
...
TEST(MERGE, merge_flat_vt) {
    ...
    faiss_navix::RandomRotationMatrix rot(d, d);
    rot.init(1234);
    faiss_navix::IndexFlatL2 quantizer(d);

    { // just to train the quantizer
        faiss_navix::IndexIVFFlat iflat(&quantizer, d, nlist);
        faiss_navix::IndexPreTransform ipt(&rot, &iflat);
        ipt.train(nb, cd.database.data());
    }

    for (int i = 0; i < nindex; i++) {
        faiss_navix::IndexPreTransform* ipt = new faiss_navix::IndexPreTransform(
                new faiss_navix::RandomRotationMatrix(rot),
                new faiss_navix::IndexIVFFlat(&quantizer, d, nlist));
        ipt->own_fields = true;
...
TEST(MERGE, merge_flat_ondisk) {
    ...
    for (int i = 0; i < nindex; i++) {
        auto ivf = new faiss_navix::IndexIVFFlat(&cd.quantizer, d, nlist);
        if (i == 0) {
            auto il = new faiss_navix::OnDiskInvertedLists(
