...
int reference_pop_min(faiss_navix::HNSW::MinimaxHeap& heap, float* vmin_out) {
    assert(heap.k > 0);
...
void test_popmin(int heap_size, int amount_to_put) {
    // create a heap
    faiss_navix::HNSW::MinimaxHeap mm_heap(heap_size);

    using storage_idx_t = faiss_navix::HNSW::storage_idx_t;
    ...
    // clone the heap
    faiss_navix::HNSW::MinimaxHeap cloned_mm_heap = mm_heap;
...
TEST(HNSW, Test_IndexHNSW_METRIC_Lp) {
    ...
    faiss_navix::IndexFlat storage_index(1, faiss_navix::METRIC_Lp);
    storage_index.metric_arg = 3;
    faiss_navix::IndexHNSW index(&storage_index, 32);
...
class HNSWTest : public testing::Test {
   protected:
    HNSWTest() {
        ...
        xb->reserve(d * nb);
        faiss_navix::float_rand(xb->data(), d * nb, 12345);
        index = std::make_unique<faiss_navix::IndexHNSWFlat>(d, M);
        index->add(nb, xb->data());
        xq = std::unique_ptr<std::vector<float>>(
                new std::vector<float>(d * nq));
        xq->reserve(d * nq);
        faiss_navix::float_rand(xq->data(), d * nq, 12345);
        dis = std::unique_ptr<faiss_navix::DistanceComputer>(
    ...
    std::unique_ptr<faiss_navix::DistanceComputer> dis;
    std::unique_ptr<faiss_navix::IndexHNSWFlat> index;
};

/** Do a BFS on the candidates list */
int reference_search_from_candidates(
        const faiss_navix::HNSW& hnsw,
        faiss_navix::DistanceComputer& qdis,
        faiss_navix::ResultHandler<faiss_navix::HNSW::C>& res,
        faiss_navix::HNSW::MinimaxHeap& candidates,
        faiss_navix::VisitedTable& vt,
        faiss_navix::HNSWStats& stats,
        int level,
        int nres_in,
        const faiss_navix::SearchParametersHNSW* params) {
    int nres = nres_in;
    int ndis = 0;
    ...
    const faiss_navix::IDSelector* sel = params ? params->sel : nullptr;

    faiss_navix::HNSW::C::T threshold = res.threshold;
    for (int i = 0; i < candidates.size(); i++) {
        faiss_navix::idx_t v1 = candidates.ids[i];
...
faiss_navix::HNSWStats reference_greedy_update_nearest(
        const faiss_navix::HNSW& hnsw,
        faiss_navix::DistanceComputer& qdis,
        int level,
        faiss_navix::HNSW::storage_idx_t& nearest,
        float& d_nearest) {
    faiss_navix::HNSWStats stats;

    for (;;) {
        faiss_navix::HNSW::storage_idx_t prev_nearest = nearest;
        ...
        for (size_t i = begin; i < end; i++) {
            faiss_navix::HNSW::storage_idx_t v = hnsw.neighbors[i];
            if (v < 0)
                break;
            ndis += 1;
            float dis = qdis(v);
...
std::priority_queue<faiss_navix::HNSW::Node> reference_search_from_candidate_unbounded(
        const faiss_navix::HNSW& hnsw,
        const faiss_navix::HNSW::Node& node,
        faiss_navix::DistanceComputer& qdis,
        int ef,
        faiss_navix::VisitedTable* vt,
        faiss_navix::HNSWStats& stats) {
    int ndis = 0;
    std::priority_queue<faiss_navix::HNSW::Node> top_candidates;
    std::priority_queue<
            faiss_navix::HNSW::Node,
            std::vector<faiss_navix::HNSW::Node>,
            std::greater<faiss_navix::HNSW::Node>>
            candidates;
    ...
    while (!candidates.empty()) {
        float d0;
        faiss_navix::HNSW::storage_idx_t v0;
...
TEST_F(HNSWTest, TEST_search_from_candidate_unbounded) {
    ...
    auto node = faiss_navix::HNSW::Node(d_nearest, nearest);
    faiss_navix::VisitedTable vt(index->ntotal);
    faiss_navix::HNSWStats stats;

    // actual version
    auto top_candidates = faiss_navix::search_from_candidate_unbounded(
            index->hnsw, node, *dis, k, &vt, stats);

    auto reference_nearest = index->hnsw.entry_point;
    float reference_d_nearest = (*dis)(nearest);
    auto reference_node =
            faiss_navix::HNSW::Node(reference_d_nearest, reference_nearest);
    faiss_navix::VisitedTable reference_vt(index->ntotal);
    faiss_navix::HNSWStats reference_stats;
...
TEST_F(HNSWTest, TEST_greedy_update_nearest) {
    ...
    // actual version
    auto stats = faiss_navix::greedy_update_nearest(
...
TEST_F(HNSWTest, TEST_search_from_candidates) {
    ...
    std::vector<faiss_navix::idx_t> I(k * nq);
    std::vector<float> D(k * nq);
    std::vector<faiss_navix::idx_t> reference_I(k * nq);
    std::vector<float> reference_D(k * nq);
    using RH = faiss_navix::HeapBlockResultHandler<faiss_navix::HNSW::C>;

    faiss_navix::VisitedTable vt(index->ntotal);
    faiss_navix::VisitedTable reference_vt(index->ntotal);
    int num_candidates = 10;
    faiss_navix::HNSW::MinimaxHeap candidates(num_candidates);
    faiss_navix::HNSW::MinimaxHeap reference_candidates(num_candidates);
    ...
    faiss_navix::HeapBlockResultHandler<faiss_navix::HNSW::C>::SingleResultHandler res(
            bres);

    res.begin(0);
    faiss_navix::search_from_candidates(
            index->hnsw, *dis, res, candidates, vt, stats, 0, 0, nullptr);
    res.end();

    faiss_navix::HNSWStats reference_stats;
    RH reference_bres(nq, reference_D.data(), reference_I.data(), k);
    faiss_navix::HeapBlockResultHandler<faiss_navix::HNSW::C>::SingleResultHandler
...
TEST_F(HNSWTest, TEST_search_neighbors_to_add) {
    ...
    std::priority_queue<faiss_navix::HNSW::NodeDistCloser> link_targets;
    std::priority_queue<faiss_navix::HNSW::NodeDistCloser> reference_link_targets;

    faiss_navix::search_neighbors_to_add(
            index->hnsw,
            *dis,
            link_targets,
...
TEST_F(HNSWTest, TEST_nb_neighbors_bound) {
    ...
    EXPECT_THROW(index->hnsw.nb_neighbors(100), faiss_navix::FaissException);
}
...
TEST_F(HNSWTest, TEST_search_level_0) {
    ...
    faiss_navix::HeapBlockResultHandler<faiss_navix::HNSW::C>::SingleResultHandler res1(
            bres1);
    RH bres2(nq, D.data(), I.data(), k);
    faiss_navix::HeapBlockResultHandler<faiss_navix::HNSW::C>::SingleResultHandler res2(
            bres2);

    faiss_navix::HNSWStats stats1, stats2;
    faiss_navix::VisitedTable vt1(index->ntotal);
    faiss_navix::VisitedTable vt2(index->ntotal);
    auto nprobe = 5;
    const faiss_navix::HNSW::storage_idx_t values[] = {1, 2, 3, 4, 5};
    const faiss_navix::HNSW::storage_idx_t* nearest_i = values;
