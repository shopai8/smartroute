...
TEST(TestZeroCopy, zerocopy_flatcodes) {
    ...
    faiss_navix::VectorIOWriter wr1;
    faiss_navix::write_index(&index1, &wr1);

    faiss_navix::VectorIOWriter wr2;
    faiss_navix::write_index(&index2, &wr2);
    ...
    faiss_navix::ZeroCopyIOReader reader(buffer.data(), buffer.size());
    std::unique_ptr<faiss_navix::Index> index1zc(faiss_navix::read_index(&reader));

    ASSERT_NE(index1zc, nullptr);

    // perform a search
    std::vector<float> cand_dis_1(k * nq);
    std::vector<faiss_navix::idx_t> cand_ids_1(k * nq);
    ...
    std::vector<faiss_navix::idx_t> cand_ids_2(k * nq);
    ...
    std::vector<faiss_navix::idx_t> cand_ids_3(k * nq);
...
TEST(TestZeroCopy, zerocopy_binary_flatcodes) {
    ...
    // make index1 and create reference results
    faiss_navix::IndexBinaryFlat index1(d);
    index1.train(nt, xt1.data());
    index1.add(nt, xt1.data());
    ...
    // make index2 and create reference results
    faiss_navix::IndexBinaryFlat index2(d);
    index2.train(nt, xt2.data());
    index2.add(nt, xt2.data());
    ...
    // serialize both in a form of vectors
    faiss_navix::VectorIOWriter wr1;
    faiss_navix::write_index_binary(&index1, &wr1);

    faiss_navix::VectorIOWriter wr2;
    faiss_navix::write_index_binary(&index2, &wr2);
    ...
    // create a zero-copy index
    faiss_navix::ZeroCopyIOReader reader(buffer.data(), buffer.size());
    std::unique_ptr<faiss_navix::IndexBinary> index1zc(
            faiss_navix::read_index_binary(&reader));
