...
TEST(TestMmap, mmap_flatcodes) {
    ...
    // create a mmap index
    std::unique_ptr<faiss_navix::Index> index1mm(
            faiss_navix::read_index(tmpname.c_str(), faiss_navix::IO_FLAG_MMAP_IFC));
...
TEST(TestMmap, mmap_binary_flatcodes) {
    ...
    // create a mmap index
    std::unique_ptr<faiss_navix::IndexBinary> index1mm(
            faiss_navix::read_index_binary(tmpname.c_str(), faiss_navix::IO_FLAG_MMAP_IFC));
