...
int main() {
    ...
    faiss_navix::Index* index_fp16;
    index_fp16 = faiss_navix::index_factory(
            d, "PQ32x4fs,Refine(SQfp16)", faiss_navix::METRIC_L2);
    index_fp16->train(nb, xb);
    index_fp16->add(nb, xb);

    // Constructing the refine PQ index with SQ8
    faiss_navix::Index* index_sq8;
    index_sq8 =
            faiss_navix::index_factory(d, "PQ32x4fs,Refine(SQ8)", faiss_navix::METRIC_L2);
    ...
    { // search xq
        ...
        float k_factor = 3;
        faiss_navix::IndexRefineSearchParameters* params =
                new faiss_navix::IndexRefineSearchParameters();
