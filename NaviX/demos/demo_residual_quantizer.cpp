...
int main() {
     ...
     ******************************************/
    using idx_t = faiss_navix::idx_t;
    size_t d = 128;
    size_t nt = 10000;
    size_t nb = 10000;
    size_t nq = 100;
    double t0 = faiss_navix::getmillisecs();

    auto tic = [t0]() {
        printf("[%.3f s] ", (faiss_navix::getmillisecs() - t0) / 1000);
    };
    ...
    faiss_navix::rand_smooth_vectors(nq + nt + nb, d, buf.data(), 1234);
    ...
    faiss_navix::knn_L2sqr(xq, xb, d, nq, nb, k, unused.data(), gt.data());

    // a function to compute the accuracy
    auto accuracy = [&](const idx_t* I) {
        idx_t accu = 0;
        for (idx_t q = 0; q < nq; q++) {
            accu += faiss_navix::ranklist_intersection_size(
    ...
    faiss_navix::ResidualQuantizer rq(
            d, 7, 6, faiss_navix::AdditiveQuantizer::ST_norm_qint8);
     ...
     ****************************************************************/
    {
        faiss_navix::IndexResidualQuantizer index(
                rq.d, rq.nbits, faiss_navix::METRIC_L2, rq.search_type);
        ...
        // override vectors
        index.codes = faiss_navix::MaybeOwnedVector<uint8_t>(raw_codes);
        index.ntotal = nb;
        ...
        std::unique_ptr<faiss_navix::FlatCodesDistanceComputer> dc(
     ...
     ****************************************************************/
    {
        ...
        faiss_navix::ResidualCoarseQuantizer rcq(rq.d, nbits);
        ...
        faiss_navix::IndexIVFResidualQuantizer index(
                &rcq, rcq.d, nlist, rq.nbits, faiss_navix::METRIC_L2, rq.search_type);
        ...
        for (std::string filled_with : {"add", "manual", "derived"}) {
            ...
            if (filled_with == "add") {
                ...
                faiss_navix::InvertedLists& invlists = *index.invlists;
                ...
                for (idx_t i = 0; i < nb; i++) {
                    ...
                    faiss_navix::BitstringReader rd(code, code_size);
                    idx_t list_no =
...

    {
        ...
        std::unique_ptr<faiss_navix::IndexIVFResidualQuantizer> index(
                faiss_navix::ivflib::ivf_residual_from_quantizer(rq, nlevel));
        ...
        for (std::string filled_with : {"add", "derived"}) {
            ...
            if (filled_with == "add") {
                ...
                faiss_navix::ivflib::ivf_residual_add_from_flat_codes(
