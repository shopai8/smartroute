...
void generate_trainset(
    ...
    ccent.resize(d * 2 * nc);
    faiss_navix::float_randn(ccent.data(), d * 2 * nc, 123);
    faiss_navix::fvec_renorm_L2(d, 2 * nc, ccent.data());
    n = nc * n_big + nc * n_small;
    x.resize(d * n);
    weights.resize(n);
    faiss_navix::float_randn(x.data(), x.size(), 1234);
    ...
    for (int index_num = WKMT_FlatL2; index_num <= WKMT_HNSW; index_num++) {
        ...
        for (int with_weights = 0; with_weights <= 1; with_weights++) {
            ...
            { // compute distance of points to centroids
                faiss_navix::IndexFlatL2 cent_index(d);
                cent_index.add(nc, centroids.data());
                std::vector<float> dis(n);
                std::vector<faiss_navix::idx_t> idx(n);
