...
int main() {
    ...
    //       faiss_navix::IndexFlatL2 coarse_quantizer (d);
    //
    ...
    faiss_navix::MultiIndexQuantizer coarse_quantizer(d, nhash, nbits_subq);
    ...
    // faiss_navix::Index * idx = faiss_navix::read_index("/tmp/trained_index.faissindex");
    faiss_navix::write_index(&index, "/tmp/trained_index.faissindex");
    ...
    { // populating the database
        ...
        std::vector<faiss_navix::idx_t> ids(nb);
    ...
    //
    //   faiss_navix::idx_t *centroid_ids = new faiss_navix::idx_t[k];
    //   float *distances = new float[k];
    //   index.quantizer->search (1, x, k, dis, centroids_ids);
    //

    faiss_navix::write_index(&index, "/tmp/populated_index.faissindex");
    ...
    { // searching the database
        ...
        std::vector<faiss_navix::idx_t> nns(k * nq);
