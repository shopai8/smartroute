...
using faiss_navix::IndexFlat;
using faiss_navix::IndexFlat1D;
using faiss_navix::IndexFlatIP;
using faiss_navix::IndexFlatL2;
using faiss_navix::IndexRefineFlat;
...
int faiss_IndexFlat_new_with(
    ...
    try {
        IndexFlat* index =
                new IndexFlat(d, static_cast<faiss_navix::MetricType>(metric));
...
int faiss_IndexRefineFlat_new(
    ...
    try {
        IndexRefineFlat* index = new IndexRefineFlat(
                reinterpret_cast<faiss_navix::Index*>(base_index));
