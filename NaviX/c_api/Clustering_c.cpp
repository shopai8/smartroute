...
using faiss_navix::Clustering;
using faiss_navix::ClusteringIterationStats;
using faiss_navix::ClusteringParameters;
...
int faiss_kmeans_clustering(
        ...
        float* q_error) {
    try {
        float out = faiss_navix::kmeans_clustering(d, n, k, x, centroids);
        if (q_error) {
            *q_error = out;
        }
        return 0;
