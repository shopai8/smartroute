...
using faiss_navix::IndexIVF;
using faiss_navix::IndexIVFStats;
using faiss_navix::SearchParametersIVF;
...
int faiss_SearchParametersIVF_new_with(
    ...
    try {
        SearchParametersIVF* sp = new SearchParametersIVF;
        sp->sel = reinterpret_cast<faiss_navix::IDSelector*>(sel);
        sp->nprobe = nprobe;
    ...
    try {
        reinterpret_cast<const IndexIVF*>(index)->copy_subset_to(
                *reinterpret_cast<IndexIVF*>(other),
                static_cast<faiss_navix::InvertedLists::subset_type_t>(subset_type),
                a1,
                a2);
    }
    CATCH_AND_HANDLE
...
FaissIndexIVFStats* faiss_get_indexIVF_stats() {
    return reinterpret_cast<FaissIndexIVFStats*>(&faiss_navix::indexIVF_stats);
}
