...
using faiss_navix::VectorTransform;

int faiss_write_index(const FaissIndex* idx, FILE* f) {
    try {
        faiss_navix::write_index(reinterpret_cast<const Index*>(idx), f);
    }
    CATCH_AND_HANDLE
}
...
int faiss_write_index_fname(const FaissIndex* idx, const char* fname) {
    try {
        faiss_navix::write_index(reinterpret_cast<const Index*>(idx), fname);
    }
    CATCH_AND_HANDLE
}
...
int faiss_read_index(FILE* f, int io_flags, FaissIndex** p_out) {
    try {
        auto out = faiss_navix::read_index(f, io_flags);
...
int faiss_read_index_fname(
        ...
        FaissIndex** p_out) {
    try {
        auto out = faiss_navix::read_index(fname, io_flags);
        *p_out = reinterpret_cast<FaissIndex*>(out);
    }
    CATCH_AND_HANDLE
}
...
int faiss_write_index_binary(const FaissIndexBinary* idx, FILE* f) {
    try {
        faiss_navix::write_index_binary(reinterpret_cast<const IndexBinary*>(idx), f);
    }
    CATCH_AND_HANDLE
}
...
int faiss_write_index_binary_fname(
        ...
        const char* fname) {
    try {
        faiss_navix::write_index_binary(
...
int faiss_read_index_binary(FILE* f, int io_flags, FaissIndexBinary** p_out) {
    try {
        auto out = faiss_navix::read_index_binary(f, io_flags);
...
int faiss_read_index_binary_fname(
    ...
    try {
        auto out = faiss_navix::read_index_binary(fname, io_flags);
        *p_out = reinterpret_cast<FaissIndexBinary*>(out);
    }
    CATCH_AND_HANDLE
}
...
int faiss_read_VectorTransform_fname(
    ...
    try {
        auto out = faiss_navix::read_VectorTransform(fname);
