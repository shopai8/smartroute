...
int faiss_IndexBinary_train(
        ...
        const uint8_t* x) {
    try {
        reinterpret_cast<faiss_navix::IndexBinary*>(index)->train(n, x);
    }
    CATCH_AND_HANDLE
}
...
int faiss_IndexBinary_add(FaissIndexBinary* index, idx_t n, const uint8_t* x) {
    try {
        reinterpret_cast<faiss_navix::IndexBinary*>(index)->add(n, x);
    }
    CATCH_AND_HANDLE
}
...
int faiss_IndexBinary_add_with_ids(
        ...
        const idx_t* xids) {
    try {
        reinterpret_cast<faiss_navix::IndexBinary*>(index)->add_with_ids(n, x, xids);
    }
    CATCH_AND_HANDLE
}
...
int faiss_IndexBinary_search(
        ...
        idx_t* labels) {
    try {
        reinterpret_cast<const faiss_navix::IndexBinary*>(index)->search(
...
int faiss_IndexBinary_range_search(
    ...
    try {
        reinterpret_cast<const faiss_navix::IndexBinary*>(index)->range_search(
                n,
                x,
                radius,
...
int faiss_IndexBinary_assign(
        ...
        idx_t k) {
    try {
        reinterpret_cast<faiss_navix::IndexBinary*>(index)->assign(n, x, labels, k);
    }
    CATCH_AND_HANDLE
}
...
int faiss_IndexBinary_reset(FaissIndexBinary* index) {
    try {
        reinterpret_cast<faiss_navix::IndexBinary*>(index)->reset();
    }
    CATCH_AND_HANDLE
}
...
int faiss_IndexBinary_remove_ids(
        ...
        size_t* n_removed) {
    try {
        size_t n{reinterpret_cast<faiss_navix::IndexBinary*>(index)->remove_ids(
...
int faiss_IndexBinary_reconstruct(
        ...
        uint8_t* recons) {
    try {
        reinterpret_cast<const faiss_navix::IndexBinary*>(index)->reconstruct(
                key, recons);
    }
    CATCH_AND_HANDLE
}
...
int faiss_IndexBinary_reconstruct_n(
        ...
        uint8_t* recons) {
    try {
        reinterpret_cast<const faiss_navix::IndexBinary*>(index)->reconstruct_n(
                i0, ni, recons);
    }
    CATCH_AND_HANDLE
}
