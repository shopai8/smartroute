...
int faiss_VectorTransform_train(
        ...
        const float* x) {
    try {
        reinterpret_cast<faiss_navix::VectorTransform*>(vt)->train(n, x);
    }
    CATCH_AND_HANDLE
}
...
float* faiss_VectorTransform_apply(
        ...
        idx_t n,
        const float* x) {
    return reinterpret_cast<const faiss_navix::VectorTransform*>(vt)->apply(n, x);
}
...
void faiss_VectorTransform_apply_noalloc(
        ...
        const float* x,
        float* xt) {
    return reinterpret_cast<const faiss_navix::VectorTransform*>(vt)->apply_noalloc(
            n, x, xt);
}
...
void faiss_VectorTransform_reverse_transform(
        ...
        const float* xt,
        float* x) {
    return reinterpret_cast<const faiss_navix::VectorTransform*>(vt)
...
void faiss_LinearTransform_transform_transpose(
        ...
        const float* y,
        float* x) {
    return reinterpret_cast<const faiss_navix::LinearTransform*>(vt)
            ->transform_transpose(n, y, x);
}

void faiss_LinearTransform_set_is_orthonormal(FaissLinearTransform* vt) {
    return reinterpret_cast<faiss_navix::LinearTransform*>(vt)->set_is_orthonormal();
}
...
int faiss_RandomRotationMatrix_new_with(
    ...
    try {
        *p_vt = reinterpret_cast<FaissRandomRotationMatrix*>(
                new faiss_navix::RandomRotationMatrix(d_in, d_out));
    }
    CATCH_AND_HANDLE
}
...
int faiss_PCAMatrix_new_with(
    ...
    try {
        ...
        *p_vt = reinterpret_cast<FaissPCAMatrix*>(new faiss_navix::PCAMatrix(
...
int faiss_ITQMatrix_new_with(FaissITQMatrix** p_vt, int d) {
    try {
        *p_vt = reinterpret_cast<FaissITQMatrix*>(new faiss_navix::ITQMatrix(d));
    }
    CATCH_AND_HANDLE
}
...
int faiss_ITQTransform_new_with(
    ...
    try {
        ...
        *p_vt = reinterpret_cast<FaissITQTransform*>(
                new faiss_navix::ITQTransform(d_in, d_out, do_pca_));
    }
    CATCH_AND_HANDLE
}
...
int faiss_OPQMatrix_new_with(FaissOPQMatrix** p_vt, int d, int M, int d2) {
    try {
        *p_vt = reinterpret_cast<FaissOPQMatrix*>(
                new faiss_navix::OPQMatrix(d, M, d2));
    }
    CATCH_AND_HANDLE
}
...
int faiss_RemapDimensionsTransform_new_with(
    ...
    try {
        ...
        *p_vt = reinterpret_cast<FaissRemapDimensionsTransform*>(
                new faiss_navix::RemapDimensionsTransform(d_in, d_out, uniform_));
    }
    CATCH_AND_HANDLE
}
...
int faiss_NormalizationTransform_new_with(
    ...
    try {
        *p_vt = reinterpret_cast<FaissNormalizationTransform*>(
                new faiss_navix::NormalizationTransform(d, norm));
    }
    CATCH_AND_HANDLE
}
...
int faiss_CenteringTransform_new_with(FaissCenteringTransform** p_vt, int d) {
    try {
        *p_vt = reinterpret_cast<FaissCenteringTransform*>(
                new faiss_navix::CenteringTransform(d));
    }
    CATCH_AND_HANDLE
}
}
