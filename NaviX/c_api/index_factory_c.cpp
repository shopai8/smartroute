...
int faiss_index_factory(
    ...
    try {
        *p_index = reinterpret_cast<FaissIndex*>(faiss_navix::index_factory(
                d, description, static_cast<faiss_navix::MetricType>(metric)));
    }
    CATCH_AND_HANDLE
}
...
int faiss_index_binary_factory(
    ...
    try {
        *p_index = reinterpret_cast<FaissIndexBinary*>(
                faiss_navix::index_binary_factory(d, description));
    }
    CATCH_AND_HANDLE
}
