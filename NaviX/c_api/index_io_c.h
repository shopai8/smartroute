...
#ifndef FAISS_NAVIX_INDEX_IO_C_H
#define FAISS_NAVIX_INDEX_IO_C_H
 ...
 * This is equivalent to `faiss_navix::write_index` when a file descriptor is
 * provided.
 */
int faiss_write_index(const FaissIndex* idx, FILE* f);

/** Write index to a file.
 * This is equivalent to `faiss_navix::write_index` when a file path is provided.
 */
int faiss_write_index_fname(const FaissIndex* idx, const char* fname);

#define FAISS_NAVIX_IO_FLAG_MMAP 1
#define FAISS_NAVIX_IO_FLAG_READ_ONLY 2
...
/** Write index to a file.
 * This is equivalent to `faiss_navix::write_index_binary` when a file descriptor is
 * provided.
 */
int faiss_write_index_binary(const FaissIndexBinary* idx, FILE* f);

/** Write index to a file.
 * This is equivalent to `faiss_navix::write_index_binary` when a file path is
 * provided.
 */
