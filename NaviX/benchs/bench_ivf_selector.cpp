...
int main() {
    ...
    faiss_navix::rand_smooth_vectors(nb + nq, d, data.data(), 1234);
    ...
    if (access(stored_name.c_str(), F_OK) != 0) {
        ...
        index.reset(faiss_navix::index_factory(d, index_key));

        double t0 = faiss_navix::getmillisecs();
        index->train(nb, xb);
        double t1 = faiss_navix::getmillisecs();
        index->add(nb, xb);
        double t2 = faiss_navix::getmillisecs();
        printf("Write %s\n", stored_name.c_str());
        faiss_navix::write_index(index.get(), stored_name.c_str());
    } else {
        printf("Read %s\n", stored_name.c_str());
        index.reset(faiss_navix::read_index(stored_name.c_str()));
    }
    faiss_navix::IndexIVF* index_ivf = static_cast<faiss_navix::IndexIVF*>(index.get());
    index->verbose = true;
    ...
    for (int tt = 0; tt < 3; tt++) {
        ...
        std::vector<idx_t> I1(nq * k);
        {
            ...
            double t3 = faiss_navix::getmillisecs();
        ...
        {
            double t2 = faiss_navix::getmillisecs();
            faiss_navix::IVFSearchParameters params;

            faiss_navix::ivflib::search_with_parameters(
                    index.get(), nq, xq, k, D2.data(), I2.data(), &params);
            double t3 = faiss_navix::getmillisecs();
            printf("search time with nullptr selector: %.3f ms\n", t3 - t2);
        }
        FAISS_NAVIX_THROW_IF_NOT(I1 == I2);
        FAISS_NAVIX_THROW_IF_NOT(D1 == D2);

        {
            double t2 = faiss_navix::getmillisecs();
            faiss_navix::IVFSearchParameters params;
            faiss_navix::IDSelectorAll sel;
            params.sel = &sel;
...
#pragma omp parallel for if (nt > 1)
        ...
        FAISS_NAVIX_THROW_IF_NOT(I1 == I3);
        FAISS_NAVIX_THROW_IF_NOT(D1 == D3);
    }
